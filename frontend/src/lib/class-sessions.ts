...
export interface ClassSession {
  ...
  /**
   * ٠٣٥ — هل محتوى هذه الحصّةِ مقفولٌ على القارئ؟
   *
   * ⚠️ **`null` تعني «لم يُسأَلْ» لا «مفتوح».** الجوابُ يُختَمُ من المُنادي دفعةً
   * واحدةً (`SessionContentAccess::stampAll()`) لأنّ المَورِدَ يعملُ مرّةً لكلِّ
   * صفّ، فسؤالُه داخلَه استعلاماتٌ بعددِ صفوفِ الجدول. والشاشاتُ التي لا تحتاجُ
   * الجوابَ لا تدفعُ ثمنَه.
   *
   * ⛔ **ولا يُعادُ اشتقاقُه هنا.** «مقفول» قرارٌ يجمعُ الحضورَ والإعفاءاتِ
   * الأربعةَ والفتحَ المدفوع، ونسخةٌ ثانيةٌ منه في TypeScript هي بالضبط ما جعلَ
   * تسجيلاً مدفوعاً غيرَ قابلٍ للفتحِ في ٠١٨: جوابٌ على الشاشةِ وآخرُ عندَ الباب.
   */
  content_locked?: boolean | null;
  /**
   * العرضُ: كم يُخصَم · كم يملكُ · ما الذي سيُفتَح · وطريقُ الشراء.
   *
   * ⚠️ **حاضرٌ على صفحةِ الحصّةِ الواحدةِ وحدَها.** جلبُه لكلِّ صفٍّ في جدولِ شهرٍ
   * أخذَ `/schedule` من ٢٣ استعلاماً إلى ٦١ — مقيسٌ، والميزانيّةُ أحمرَت.
   *
   * ⚠️ **و`opens` أعدادٌ وأصنافٌ لا عناوين**: قائمةُ عناوينِ أوراقِ ساعةٍ هي
   * خطّةُ درسٍ لمن لم يحضرْها.
   *
   * ⚠️ **ولا مبلغَ نقديٍّ فيه إطلاقاً.** ثمنُ الحصّةِ هو أجرُ المدرّسِ مضافاً
   * إليه ثابتانِ، فرقمٌ على شاشةِ الطالبِ يُحَلُّ للآخر (FR-021ج).
   */
  content_offer?: SessionContentOffer | null;
}

/** `App\Shared\Data\SessionContentOffer` حقلاً بحقل. */
export interface SessionContentOffer {
  /** دائماً ١ — موافقةٌ واحدةٌ تفتحُ الساعةَ كلَّها، لا تسعيرَ لكلِّ عنصر. */
  credits: number;
  owned_credits: number;
  /** المملوكُ ناقصَ المُجمَّدِ لحصصٍ أخرى. **هذا** ما يُقارَنُ بالثمن. */
  available_credits: number;
  /** `{ lesson: 3, assignment: 1 }` — أصنافٌ وأعداد. */
  opens: Record<string, number>;
  /** متى تنتهي إتاحةُ المادّة (FR-039ب) — `null` لا انتهاءَ معلوم. */
  available_until: string | null;
  /**
   * طريقُ الشراء — **دائماً، لا حينَ يَعجِزُ الرصيدُ وحدَه**.
   *
   * حمولةٌ يتغيّرُ شكلُها بما وجدَته جوابٌ مُميِّز، والرفضُ بلا مخرجٍ هو الشكلُ
   * الذي تمنعُه FR-013.
   */
  purchase_url: string;
}
...
export const classSessions = {
  ...
  /**
   * ٠٣٥ · FR-010 — موافقةٌ واحدةٌ تفتحُ كلَّ محتوى الحصّةِ بخصمِ حصّةٍ واحدة.
   *
   * ⚠️ **والأجوبةُ أربعةٌ لا اثنان**، وكلٌّ منها جملةٌ مختلفةٌ على الشاشة:
   * `200` فُتِح · `409` مفتوحٌ لك بالفعل (لا خصمَ ثانٍ أبداً — FR-011) ·
   * `422` لا يكفي رصيدُك، **ومعه العرضُ نفسُه** ليُرى طريقُ الشراء ·
   * `403` ليست لك.
   */
  unlock: (uuid: string) =>
    api.post<{ uuid: string; credits_charged: number; content_locked: false }>(
      `/class-sessions/${uuid}/unlock`,
      {},
    ),
