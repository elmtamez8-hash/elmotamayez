 ...
 *
 * ⚠️ **لا صورةَ لكلِّ مقال، ولم تُخترَعْ واحدة.** `cms_articles` لا عمودَ غلافٍ
 * فيها أصلاً — لا في الهجرةِ ولا في الحمولة — فبطاقةٌ بمساحةِ صورةٍ فارغةٍ أو
 * بصورةٍ عامّةٍ مكرَّرةٍ اثنتَي عشرةَ مرّةً أسوأُ من بطاقةٍ بلا صورة. الأيقونةُ
 * زخرفةٌ مُعلَنة (`aria-hidden`)، لا تدّعي تصنيفاً.
 *
...
export function ArticleCard({
  ...
  return (
    <article
      className={`group relative flex h-full flex-col rounded-3xl border border-line bg-surface-raised p-6 transition duration-200 ease-out hover:-translate-y-1 hover:border-primary/40 hover:shadow-lg active:translate-y-0 active:duration-100 ${
        featured ? "sm:p-8" : ""
      }`}
    >
