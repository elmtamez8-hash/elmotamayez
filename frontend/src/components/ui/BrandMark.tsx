...
export function BrandMark({ size = "md" }: { size?: keyof typeof SIZES }) {
  return (
    <span className={`wordmark ${SIZES[size]}`} role="img" aria-label={usePlatformName()} />
  );
}
 ...
 */
export function BrandMarkDecorative({ size = "md" }: { size?: keyof typeof SIZES }) {
  return <span className={`wordmark ${SIZES[size]}`} aria-hidden="true" />;
}
