...
class DataCategorySeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (! TranslatableColumns::converted('data_categories', 'label')) {
            return;
        }
    ...
    public static function categories(): array
    {
        return [
            // ── Identity ────────────────────────────────────────────────────
            /*
            | ⛔ الرابطُ بينَ وليِّ الأمرِ وطفلِه — ولم يكنْ له صفٌّ في الكتالوجِ
            | إلى ٢٠٢٦-٠٩-١٦، وهو بياناتٌ شخصيّةٌ عن اثنَينِ لا عن واحد.
            |
            | ⚠️ **وأثرُ غيابِه مكتوبٌ في `IdentityPersonalData` بخطِّ مَن كتبَه**:
            | مشيُ التصديرِ كانَ يُودِعُ هذه الصفوفَ تحتَ مفتاحِ `student_name`،
            | «لأنّ مفتاحاً لا يُصرِّحُ به صفٌّ في `data_categories` ملفٌّ بلا
            | قاعدةِ حفظٍ وبلا مالك». فصارَ له مفتاحُه.
            |
            | ⚠️ **و`student_name` في هذا الجدولِ اسمٌ يكتبُه وليُّ الأمرِ بيدِه عن
            | طفلٍ قد لا يملكُ حساباً أصلاً** (العمودُ `student_user_id` قابلٌ
            | للفراغِ عن قصد). فهو الحقلُ الأحدُّ هنا، ولذلك يُحذَفُ الصفُّ كاملاً
            | ولا يُجهَّل: تجهيلُ صفٍّ نصفُه اسمٌ حرٌّ لا يُجهِّلُ شيئاً.
            |
            | و`subject_roles` طرفانِ لا طرف: الطالبُ ووليُّ أمرِه كلاهما موصوفٌ
            | في الصفِّ نفسِه، وإخفاؤُه عن أحدِهما إخفاءٌ لعلاقةٍ هو نصفُها.
            |
            | ⛔ **و`erasure_mode` هنا `Anonymise` لا `Delete`، وكتابةُ `Delete`
            | أطفأَتِ المسحَ في وحدةِ `Identity` كلِّها.** `ExecuteDataErasure::modeFor()`
            | يجمعُ أوضاعَ فئاتِ الوحدةِ ويطلبُها **مُجمَعةً**: فإن اختلفَت سجَّلَ
            | `unclear_mode` ورجعَ بـ`Retain` — فلا اسمٌ يُجهَّلُ ولا بريدٌ ولا
            | هاتف. قِيسَ: خمسُ حالاتٍ انقلبَت حمراءَ، ورسالتُها «الاسمُ ما زالَ
            | Zubaida» لا «الوضعُ مختلف».
            |
            | والوضعُ يصفُ ما يقعُ **بصاحبِ البيانات**، لا بكلِّ صفٍّ على حدة:
            | تجهيلُ الحسابِ يشملُ حذفَ صفوفٍ لا يُجهِّلُها شيء — وهو ما كانَ
            | `IdentityPersonalData` يفعلُه بهذه الصفوفِ أصلاً تحتَ `student_name`.
            */
            [
                'key' => 'guardian_link',
                'subject_roles' => ['student', 'parent'],
                'label' => 'ارتباطك بوليِّ أمرك — أو بطفلك',
                'purpose' => 'لنعرف من يحقُّ له متابعة دراستك، وبأيِّ صلاحيّة: المواعيد أو النتائج أو الحضور. ويشمل ما كتبه وليُّ الأمر عن طفله عند الربط — الاسم والسنّ والصفّ — حتى قبل أن يكون للطفل حساب.',
                'audience' => 'وليّ الأمر · الطالب · إدارة المنصّة',
                'is_required' => true,
                'owning_module' => 'identity',
                'table_name' => 'parent_student_relations',
