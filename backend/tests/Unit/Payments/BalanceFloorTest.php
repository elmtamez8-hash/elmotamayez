...
it('withholds at the default state of zero credits and zero limit', function (): void {
    ...
    expect($this->ledger->isBlocked($balance, $this->ledger->floorFor($balance, BillingMode::PrepaidCredits)))
        ->toBeTrue();
});
...
it('does not withhold inside an unused ceiling', function (): void {
    ...
    expect($this->ledger->isBlocked($balance, $this->ledger->floorFor($balance, BillingMode::ManualCollection)))
        ->toBeFalse();
});
