...
class ReadPublicCourse extends Action
{
    ...
    */
    ...
    /**
     * هل اكتملَ الكورس؟ (٠٣٤ · FR-023 · T051)
     *
     * ⚠️ **مُفوَّضةٌ إلى الدليل، لا مشتقّةٌ من المجموعاتِ المحمَّلةِ فوقَها.**
     * الاشتقاقُ هنا ممكنٌ — `status !== 'archived' && seats_left !== 0` — وهو
     * **إملاءٌ ثانٍ** للشرطِ الذي يحرسُ بابَ الشراء: بطاقةٌ تقولُ «مكتمل» وبابٌ
     * يبيع، أو العكس. والفرقُ ليسَ نظريّاً: قائمةُ المجموعاتِ العامّةُ مُرشَّحةٌ
     * على الملكيّةِ لا على الحالة، و«صالحةٌ للانضمام» (مفتوحةٌ وغيرُ مكتمِلة)
     * غيرُ «صالحةٍ للإسناد» (غيرُ مؤرشَفةٍ وغيرُ مكتمِلة) — وهما اللذانِ اختلطا
     * أربعَ مرّاتٍ قبلَ FR-030.
     *
     * والكلفةُ استعلامانِ مفهرسانِ على صفحةٍ تقرأُ أضعافَهما، والثمنُ المقابلُ
     * شاشةٌ تُخالِفُ البابَ الذي تُرسِلُ إليه.
     */
    public function isFull(Course $course): bool
    {
        return $this->cohorts->courseIsFull((int) $course->getKey());
    }

    /**
     * Whether «سجّل مجاناً» may be drawn — the free-enrolment door's OWN question.
     *
     * ⛔ `Course::isFree()` — the teacher's explicit «كورس مجاني» and nothing
     * else (owner decision 2026-09-25), never a price and never the absence of
     * a plan. `EnrollmentController::enroll()` refuses on exactly this helper,
     * so the button and the door agree.
     */
