                ...
                foreach ($inChapter as $lesson) {
                    ...
                    */
                    if ($lesson->isPubliclyReadable()) {
                        ...
                        $item['is_open'] = true;
                    } elseif ($lesson->isOpen()) {
                        /*
                        | ⛔ **درسٌ فتحَه المدرّسُ عمداً وكانت الصفحةُ العامّةُ
                        | تقولُ عنه «بعد التسجيل» — وهذا عطلٌ لا نقصُ ميزة.**
                        |
                        | `isPubliclyReadable()` أعلاه تشترطُ `embed`، وهو شرطٌ
                        | صحيحٌ لأنّه يقرّرُ **بناءَ الرابط**: معرّفُ فيديوٍ
                        | مرفوعٍ في حمولةٍ عامّةٍ دعوةٌ إلى نقطةِ التشغيل. فكانَ
                        | الفيديو المرفوعُ الموسومُ «متاح بلا تسجيل» يسقطُ إلى
                        | فرعِ القفلِ كأنّ المدرّسَ لم يوسِمْه أصلاً — الوسمُ له
                        | قارئٌ عندَ **المنح** ولا قارئَ عندَ **الإعلان**.
                        |
                        | وهذا المفتاحُ يُعلِنُ ولا يفتح: **بلا `uuid`**، فلا
                        | رابطَ يُبنى ولا شيءَ يُجرَّبُ على نقطةِ التشغيل، والجملةُ
                        | المكتوبةُ في المستودعِ كلِّه — «شكلُ البيانات هو ما
                        | يمنعُ بناءَ الرابط» — تبقى صحيحةً بحرفِها.
                        |
                        | والاسمُ `free_with_account` لا `is_free`: العمودُ
                        | `lessons.is_free` يعني «بلا مقابلٍ داخلَ الكورس»،
