...
class PublicTeacherDetailResource extends PublicTeacherCardResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            // Both filled in by the controller, which holds the Actions that read
            // them. The defaults here are what a profile with neither looks like,
            // so a caller that forgets to merge gets "nothing yet", not a crash.
            'courses' => [],
            'reviews' => [
            ...
            /*
             * ⚠️ كانَ `[]` مكتوباً حرفيّاً هنا، والمفتاحُ في القائمةِ البيضاءِ منذُ
             * ٠٠٢ و`FaqAccordion` مرسومٌ خلفَ `length > 0` على الصفحة — فلم يُعرَضْ
             * قطُّ ولم يفشلْ شيء. عمودٌ له قارئٌ وبلا كاتب، مقلوباً.
             */
            'faqs' => $this->faqs ?? [],
