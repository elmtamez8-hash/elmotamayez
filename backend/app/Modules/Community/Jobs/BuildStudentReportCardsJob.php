...
/**
 * Build the cumulative report cards of ONE chunk of students (FR-036, FR-041).
 *
 * Dispatched by {@see BuildReportCardsJob}, which enumerates the period's
 * (student, workspace) pairs and hands out {@see BuildReportCardsJob::STUDENTS_PER_JOB}
 * students per job. Every workspace of a student arrives in the SAME chunk — see
 * the parent for why a student is never split.
 *
 * ⚠️ SAFE TO RUN TWICE, AND THAT IS WHAT MAKES A FAILED CHUNK RECOVERABLE. The
 * card is claimed by its unique key (`claimCard()`), a published card is skipped,
 * and the publish is a conditional UPDATE only one run can win — so re-dispatching
 * the whole period re-does exactly the cards that are not finished.
 *
 ...
 */
class BuildStudentReportCardsJob implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    /**
     * @param  array<int, list<int>>  $workspaceIdsByStudent  student id => the workspaces they studied in
     */
    public function __construct(
        private readonly string $periodStart,
        private readonly string $periodEnd,
        public readonly array $workspaceIdsByStudent,
    ) {
    ...
    public function handle(
        ...
        foreach ($this->workspaceIdsByStudent as $studentId => $workspaceIds) {
