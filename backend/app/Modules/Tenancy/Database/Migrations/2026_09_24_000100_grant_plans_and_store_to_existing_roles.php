...
/**
 * Create `plans.manage`, `store.items.manage` and `store.shipments.manage` and
 * give them to every EXISTING role the matrix says holds them.
 *
 * Spec 011 added all three to the matrix and shipped no migration like this one,
 * so on production (2026-09-24) the teacher opening «باقات الاشتراك» read «هذه
 * الصفحة ليست لك» — a teacher could not sell a private-sessions plan at all.
 *
 * ⚠️ THE SAME TRAP AS `chat.reply`, `chat.moderate`, `reviews.periodic.manage`
 * AND `announcements.manage`: the matrix is a SEED that `SeedDefaultRoles` runs exactly once, when
 * a workspace is created — so adding a constant reaches nobody who already exists,
 * while the suite stays green because every fixture creates its workspace after
 * the change.
 *
 * ⚠️ AND THE ROLES SCREEN BITES FIRST. Its vocabulary is
 * `RolePermissionMatrix::tenantPermissions()`, derived from CODE — so the new box
 * renders the moment this deploys, and ticking it on a database with no such row
 * raises `PermissionDoesNotExist`: a 500 on the owner's own settings screen.
 *
