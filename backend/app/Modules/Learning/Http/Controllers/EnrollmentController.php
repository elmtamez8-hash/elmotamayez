...
class EnrollmentController extends Controller
{
    ...
    public function showLessonForViewer(Request $request, string $lessonUuid): JsonResponse
    {
        ...
        /*
        | ⛔ **مالكُ المنصّةِ يفتح، والبابانِ يتحرّكانِ معاً.**
        |
        | `Gate::before` يُمرِّرُ `users.is_super_admin` فوقَ كلِّ سياسةٍ في
        | المنتَج — وهذانِ البابانِ يسألانِ العضويّةَ مباشرةً لا سياسةً، فكانا
        | الاستثناءَ الوحيد: المالكُ يفتحُ أيَّ شاشةٍ في المنصّةِ ويُردُّ عن
        | صفحةِ الدرسِ وحدَها بـ«لا تملك تسجيلاً في هذا الكورس».
        |
        | ⚠️ **ويُقرَأُ هنا وفي `IssuePlaybackGrant::mayWatch()` معاً**، وإلّا
        | انفتحتِ الصفحةُ ورفضَ الفيديو — وهو بعينِه عطبُ ٠١٨: `mayWatch()` يقولُ
        | نعم والتسلسلُ يقولُ لا، فصارَ تسجيلٌ مدفوعٌ غيرَ قابلٍ للفتحِ إطلاقاً.
        |
        | ⚠️ **وسقفٌ مكتوبٌ لا مخبوء**: `LessonAudience` لا يعرفُ هذا الاستثناءَ،
        | فشجرةُ المنهجِ قد تُسقِطُ عن المالكِ صفّاً مقصوراً على مجموعة. البابُ
        | المقصودُ مفتوحٌ، والقائمةُ لم تُوسَّعْ — قرارُ المالكِ ٢٠٢٦-٠٩-١٦.
        */
        ...
        */
        ...
        /*
        | ⚠️ **والرمزُ هو ما يصلُ إلى الشاشة، لا الجملة.** `userMessage()` يطرحُ
        | نصَّ كلِّ ٤٠٤ ويطبعُ «العنصر المطلوب غير موجود أو حُذف» — وهو حارسٌ
        | صحيحٌ (٤٠٤ الإطاريُّ إنجليزيٌّ ولا يصلحُ لشاشة) بثمنٍ خاطئ: قيلَ للمالكِ
        | إنَّ المحتوى **حُذِف** بينما هو موجودٌ ولا يخصُّه. و`BY_CODE` هي آليّةُ
        | ذلكَ الملفِّ نفسِها: رمزٌ يُوثَقُ به، لا نصٌّ يُمرَّر.
        */
        return response()->json([
            'message' => 'لا تملك تسجيلاً في هذا الكورس.',
            'code' => 'not_enrolled',
