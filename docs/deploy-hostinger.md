...
### والنشرةُ تتوقّفُ إن كانت مجموعةٌ فيها طلابٌ ستخرجُ من العرض

قبلَ رفعِ الخدماتِ يُشغَّلُ `php artisan cohorts:gate-impact` من **الصورةِ
الجديدةِ** في حاويةٍ عابرة. يقرأُ ولا يكتبُ، ويجيبُ برقمٍ واحد: كم مجموعةً فيها
أعضاءٌ لا يصلُها سعرٌ حيّ، أي كم غرفةً قائمةً ستصيرُ غيرَ مرئيّةٍ للحظةِ النشر.

- **صفر** ⇒ تمضي النشرةُ بلا أثر.
- **أكثرُ من صفر** ⇒ تتوقّفُ النشرةُ ويُطبَعُ جدولٌ بالمجموعاتِ وأصحابِها.

وهو حارسٌ لا تقرير: بعدَ `up -d` يكونُ الطلابُ قد فقدوا مجموعتَهم فعلاً. فإن
قُرِئَ الجدولُ وقُبِلَ الأثرُ — أو كانَ التعديلُ عاجلاً لا علاقةَ له بالباقات —
تُمرَّرُ النشرةُ صراحةً:

```bash
ssh root@<IP> 'cd /srv/elmotamayez && SKIP_COHORT_GATE_CHECK=1 ./scripts/deploy.sh'
```

والعلاجُ الحقيقيُّ أسرعُ عادةً: تُسعَّرُ باقةُ تلكَ المجموعةِ من
`/admin/plans`، ويُعادُ تشغيلُ النشرةِ بلا متغيّر.
...
13. ⚠️ **`$exists->select(1)` يُلَفُّ كاسمِ عمود.** لارافيل يعاملُ وسيطَ
    `select()` اسمَ عمود، فيخرجُ `` select `1` ``: MySQL يرُدُّ
    `1054 Unknown column '1'`، و**SQLite يقبلُ معرِّفاً مقتبَساً لا يطابقُ
    عموداً ويعاملُه نصّاً**. والدالّةُ (`ReferenceIntegrity`) داخلَ
    `progressEligible`، أي في **مقامِ نسبةِ الإنجاز** وقراءاتِ الطالب — فمادّةٌ
    فيها امتحانٌ أو حصّةٌ ترُدُّ ٥٠٠، و`CourseCompleted` لا يُطلَقُ، ولا تصدرُ
    شهادةٌ أبداً. حارسُه يقيسُ **نصَّ SQL**، لأنّ لا فحصٍ سلوكيٍّ على SQLite
    يستطيعُ رؤيتَه.
14. ⚠️ **`SCOUT_MEILISEARCH_HOST` مفتاحٌ لا يقرؤه أحد** — `config/scout.php`
    يقرأُ `MEILISEARCH_HOST`، فيسقطُ إلى `localhost:7700` داخلَ الحاوية. كلُّ
    كتابةٍ لنموذجٍ مفهرَسٍ تفشل. ثالثُ مفتاحٍ ميّتٍ بعدَ مفتاحَي Bunny.
...
## ١٣ · بذرُ الإنتاج وحساباتُ العرض

🤖 **المرجعيّةُ (٧)** عبرَ `php artisan db:seed --force`: أدوارٌ وصلاحيّاتٌ
وإعداداتُ منصّةٍ وقوالبُ إشعارٍ وباقاتُ رصيدٍ وفئاتُ بياناتٍ ومعالِجون وأفعالُ
تحفيز. **آمنةٌ ومتكرِّرة** — و`DatabaseSeeder` يحملُ حارسَ
`if (! app()->environment('production'))` حولَ البياناتِ التجريبيّة، فلا تُشغَّلُ
إلّا بـ`--class=` صراحةً.

🤖 **التجريبيّةُ الخفيفةُ (٥)**: `DemoData` · `Scenario` · `Marketplace` ·
`CohortDemo` · `StudentDashboard`.

⚠️ **`MarketplaceLoadSeeder` لم يُشغَّلْ ولا يُشغَّل**: ٥٠٠٠٠ مدرّسٍ منشورٍ
و١٠٠٠٠ مادّة — تجهيزُ قياسِ أداءٍ (SC-008) لا بياناتِ عرض. يغمرُ المتجرَ العامَّ
ويُفهرِسُه جوجل، وحذفُه نظيفاً صعبٌ لأنّه إدراجٌ خامٌّ بلا مفاتيحَ ثابتة.

⚠️ **والمتجرُ العامُّ يبقى فارغاً — عمداً.** `MarketplaceSeeder::run()` يستدعي
`seedDemoMarketplace()` خارجَ الإنتاجِ فقط، وتوثيقُه يقول: «نشرُ مساحةِ عملٍ
حقيقيّةٍ على المتجرِ قرارُ اختيارٍ صريح (FR-001) لا شيءٌ يفعلُه سيدرٌ نيابةً عنك».
فالمعروضُ للجمهورِ يبقى صفراً حتى يُعتمَدَ مدرّسٌ من `/admin` وتُفعَّلَ مشاركةُ
مساحتِه. لم أتجاوزْ ذلك.

🤖 **كلماتُ مرورِ العرض**: كانت `password` حرفيّاً ببريدٍ مثل
`teacher@example.com` — وهو حسابٌ يملكُ مساحةَ عمل، و`/login` مفتوحٌ للإنترنت.
بُدِّلت الستَّ عشرةَ بعشوائيّةٍ، والإثباتُ على الحساباتِ نفسِها (`Hash::check`)
لا على عدّادِ ما كُتِب: **صفرُ حسابٍ يقبلُ `password`**.

    ssh root@<server> 'cat /root/elmotamayez-demo-logins.txt'   # 600

## ١٤ · بابُ `/admin`

مديرُ المنصّةِ (`users.is_super_admin`) **أو** حاملُ صفٍّ في `platform_staff`.
لا مالكَ مساحةٍ ولا مدرّسَ ولا مساعد — للمدرّسِ سطحُه الكاملُ في `/manage/*`.

⚠️ **والثمنُ مكتوب**: مالكُ المساحةِ لم يعُدْ يفتحُ `/admin/shield/roles`.
`RoleResource::canViewAny()` ما زال صادقاً له — الصلاحيّةُ لم تتغيّرْ، البابُ
فوقَها تغيّر. إدارةُ المساعدينَ باقيةٌ في `/manage/assistants`.
