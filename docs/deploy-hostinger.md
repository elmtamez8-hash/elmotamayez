...
🤖 **اثنا عشرَ**، ولا واحدَ منها كان ليظهرَ في بوّابةٍ محلّيّة: جهازُ التطويرِ
...
12. ⚠️ **nginx يحفظُ عنوانَ الحاويةِ إلى الأبد.** يحلُّ اسمَ كلِّ خادمٍ أعلى
    مرّةً واحدةً عندَ الإقلاع، وDocker يمنحُ عنواناً جديداً لكلِّ حاويةٍ تُعادُ
    إنشاؤها. قِيسَ هنا: nginx أقلعَ ١٠:٤٢:٣٥ فحلَّ `reverb` إلى `172.19.0.9`،
    وأُعيدَ إنشاءُ `reverb` ١٠:٤٧:٤٥ فصارَ `172.19.0.6` ⇒
    `connect() failed (111: Connection refused)` على **كلِّ** وصلةِ سوكِت.
    ⚠️ **والعرَضُ الوحيدُ «النقاشُ لا يظهرُ لحظيّاً»** — لا خطأَ في الصفحةِ ولا
    في سجلِّ التطبيق. والفحصُ الدخانيُّ أعمى: `/app` بطلبٍ عاديٍّ يرُدُّ ٥٠٠ من
    Reverb نفسِه (ليس ترقيةَ WebSocket) سليماً كان أو معطوباً. لم يظهرْ إلّا
    بمصافحةٍ حقيقيّةٍ من عميلِ WebSocket. و`backend`/`frontend` على الفخِّ نفسِه —
    نجَوا بالصدفةِ لأنّ ترتيبَ الإقلاعِ وافق. العلاجُ `resolver 127.0.0.11`
    **مع متغيّرٍ** في `proxy_pass`/`fastcgi_pass` و`$request_uri` صريحاً.

---

## ١١ · بنودٌ مفتوحة

- ⚠️ **أسرارُ المزوّدين (LiveKit · R2 · Bunny)** — انظر §١٢. أسرارُ الخادمِ
  نفسِه دُوِّرَت؛ هذه وحدَها تنتظرُ لوحاتِ المزوّدين.
  **النشرةُ تعملُ بدونِها؛ الفيديو والبثُّ وحدَهما لا يعملان.**
- **حسابُ المدير**: أُنشِئ، وكلمتُه في `/root/elmotamayez-admin.txt` (`600`).
  ⚠️ **تُغيَّرُ عندَ أوّلِ دخول** ويُحذَفُ الملفّ — كلمةٌ في ملفٍّ نصّيٍّ تبقى
  صالحةً بعدَ أن ينساها الجميع.
- **البريدُ غيرُ مُهيَّأ** (`MAIL_MAILER=smtp` بلا مضيف)، فاستعادةُ كلمةِ المرورِ
  وتوثيقُ البريدِ **لا يعملان** — وهما أصلا مصادقةٍ لا إشعارُ نطاق. يعني: من
  ينسى كلمتَه لا سبيلَ له إلّا `tinker` على الخادم.
