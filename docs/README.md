...
| PUT | `/me/timezone` | The clock this account reads (`users.timezone`), which every server-printed time is formatted on (`UserClock`). `source: manual` (the settings card — the source of truth) or `source: browser` (the sign-in stamp; `only_if_unset: true` is the older spelling of it). A browser write is a conditional UPDATE that never overwrites a manual choice, and follows the browser until one is made (owner decision 2026-09-26). Validated against PHP's zone list |
...
| GET | `/schedule/children?student={uuid}` | sanctum + the guardian relation carrying `schedule`. **A separate route, never an optional `?student=` on the line above** — that one's authorisation IS the absence of a parameter, and a guard which is usually skipped is a guard nobody exercises. `?student=` is REQUIRED (`422` without it) and matched INSIDE the authorised list rather than fetched then checked, since a bare uuid is an identity probe. `403` and never `404`, byte-identical for «not your child» and «nobody». It calls `GetStudentSchedule` unchanged — a guardian stands in for the CHILD, not for a teacher — and answers a NARROW resource: no `join_open` (an invitation into a room they cannot enter), no `my_booking` (computed about the reader, so `null` about a seat their child holds), no `recording` (a link that answers 404), no `seats`, no `unlock_*` |
| GET | `/attendance/children/summary?student={uuid}&days=30` | sanctum + the guardian relation carrying `attendance`. Four counts, their total and a rate — **not the register**, which is `attendance.view` and carries names, stays and the reason a mark changed. Crosses workspaces deliberately: a child studying with three teachers has ONE attendance record, so the scope is dropped on both the aggregate and its session subquery. `rate_pct` is `(total − absent) / total` (an excusal counts as attended, as everywhere else) and is **`null`, never `0`, when nothing has been recorded** — «lessons have not started» and «missed every one» are opposite sentences |
