...
| PUT | `/me/timezone` | The clock this account reads (`users.timezone`), which every server-printed time is formatted on (`UserClock`). The browser stamps its own zone on sign-in with `only_if_unset: true` — a conditional UPDATE, so a zone somebody chose is never overwritten. Validated against PHP's zone list |
