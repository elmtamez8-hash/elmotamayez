...
- [x] T108 [US4] `BuildSelfExam` في `.../Actions/BuildSelfExam.php`: الترشيح بالفكرة والصعوبة والعدد والمدّة، **وبالمتاح مع إبلاغ الطالب عند النقص** (`FR-023`)
- [x] T109 [US4] ⚠️ **[‏مر‏]** في `BuildSelfExam`: بِركة السحب = أسئلة الدروس التي يخوّلها **تسجيلٌ نشط** (`EnrollmentDirectory`) — **لا عضويةُ مساحة**، فطالبٌ انتهى تسجيله وبقيت عضويته يقرأ البنك كاملاً (`FR-022`)
- [x] T110 [US4] ⚠️ **[‏مر‏]** في `BuildSelfExam`: **اطرح كل سؤالٍ في `exam_items` لاختبارٍ منشور لم يُكمل الطالب فيه محاولة** (`FR-022أ`) — بدونه يستدعي الطالب فكرةَ امتحان الغد وصعوبته، فيُصحَّح له فوراً **مع الشروح**، ويقرأ الامتحان بإجاباته
- [x] T111 [US4] المحاولة المُولَّدة تُوسَم `is_practice` و`exam_id = null` — الوسم **على المحاولة لا على الاختبار**، لأن الاختبار نفسه قد يُحلّ رسمياً وتدريباً (`FR-025`)
- [x] T112 [P] [US4] `BuildSelfExamRequest` و`PracticeAttemptResource`
- [x] T113 [US4] `PracticeController` والمسار `‏/practice/exams` بـ`throttle:practice`
- [x] T114 [US4] التصحيح الآليّ الفوريّ مع الشروح للاختبار الذاتي (`FR-024`) في `GradeAttempt`
- [x] T115 [P] [US4] `frontend/src/app/(app)/(shell)/practice/page.tsx` — الاختيار والنتيجة والشروح
- [x] T116 [US4] ⚠️ **رابطٌ وارد** للتدريب من قائمة الطالب
- [x] T117 [P] [US4] `backend/tests/Feature/Assessments/SelfExamTest.php` — مطابقة المعايير في ‎١٠٠٪‎، والنقص يُبنى بالمتاح مع إبلاغ (`SC-009`)
- [x] T118 [P] [US4] ⚠️ **[‏مر‏]** `backend/tests/Feature/Assessments/ExamLeakTest.php` — امتحانٌ منشور، والطالب يطلب تدريباً **بالمعايير نفسها** ⇒ **لا سؤالٌ منه يظهر**؛ ثم يُكمل محاولته الرسمية ⇒ تدخل البِركة (`SC-020`)
- [x] T119 [P] [US4] ⚠️ **[‏مر‏]** `backend/tests/Feature/Assessments/PracticeBudgetTest.php` — اختبارٌ بمحاولتين، تدريبٌ ثلاث مرّات، ثم **محاولتان رسميّتان تُقبلان** (`SC-020`)
...
- [X] T180 ⚠️ **[‏مر‏]** **هجرة ‎٦‎ في نشرةٍ تالية منفصلة**: احذف `questions.exam_id` في `..._000600_drop_exam_id_from_questions.php` — عاملُ طابورٍ قديم لم يُعَد تشغيله ينفّذ `$attempt->load('exam.questions.options')` فيسقط **كل** تصحيحٍ وكل صفحة اختبار حتى تكتمل النشرة
  - نُفِّذت في `2026_08_26_000600_drop_exam_id_from_questions.php` بعد **تسع نشرات**: ‎٠٠٩‎ · ‎٠١٠‎ · ‎٠١٣‎ · ‎٠١٤‎ · ‎٠١٥‎ · ‎٠١٦‎ · ‎٠١٧‎ · ‎٠١٩‎ · ‎٠٢٠‎. الشرطُ المسبق قِيس ولم يُفترَض: `Exam::questions()` صارت `belongsToMany` عبر `exam_items` · العمود خارج `$fillable` منذ ‎٠٠٨‎ · و`grep` على `questions.exam_id` لا يُرجع إلا تعليقات — وكلُّ `exam_id` باقٍ يخصّ `exam_items` أو `exam_attempts`.
  - ⚠️ **والفهرسُ يُحذَف أوّلاً في جملةٍ مستقلّة**: `questions_exam_id_index` وُلد مع الجدول، وMySQL يُسقط فهرسَ العمود الواحد معه بلا اكتراث — بينما `ALTER TABLE … DROP COLUMN` في SQLite **يرفض عموداً مفهرساً**، والمجموعةُ كلُّها تعمل على SQLite في الذاكرة. الصيغةُ التي تبدو زائدةً هي الوحيدةُ التي تعمل أصلاً.
- [X] T181 في `T180`: **اكتب حدود التراجع الثلاث في الهجرة** — سؤالٌ في اختبارين لا يتراجع · سؤالُ بنكٍ في صفر اختبارات لا قيمة له · والعمود **لا يعود `NOT NULL`**، فالمخطّط بعد `rollback` ليس الذي سبق
  - الثلاثةُ مكتوبةٌ فوق `down()` الفارغة: عمودٌ يُعاد فارغاً أسوأُ من عمودٍ غائب، لأنّه يُقرَأ كبيانٍ ولا يحمل شيئاً. التراجعُ خلف هذه النقطة استعادةُ نسخةٍ احتياطيّة، وقولُها هنا أرخصُ من اكتشافها من شاشةِ تصحيح.
