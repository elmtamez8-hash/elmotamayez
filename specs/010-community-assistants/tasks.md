...
- [X] T175 ✅ **رُوجعت وسقطت، والسببُ مكتوب**: لا صنفَ `Archive` في هذه الوحدةِ أصلاً — أصنافُها الأربعةُ كلُّها `Delete`، وحذفُ الكشفِ يمرُّ بالنموذجِ (`deleteCards()`) فتتعاقب medialibrary على الـPDF. علامةُ الأرشفةِ حارسٌ لسلوكٍ غيرِ موجود. النصُّ الأصليّ: ⚠️ **راجعْ جدواها أوّلاً**: المرحلةُ ٧ صنّفت `report_card` صنفَ `Delete` والحذفُ يمرُّ بالنموذجِ فتتعاقب medialibrary على الملفّ — فقد لا يبقى صنفُ `Archive` في هذه الوحدةِ أصلاً. وإن بقي: أضف علامةَ أرشفةٍ لملفِّ الكشفِ تحت الصنفِ `Archive` — ⚠️ بلا علامةٍ تُعاد أرشفتُه كلَّ ليلةٍ ويُحذَف الملفُّ عند المزوّدِ مرّةً بعد مرّة (‏سابقةُ `media_assets.archived_at`)
- [X] T176 ⚠️ **و`announcements` معها**: المرحلةُ ٨ أضافت جدولاً يحمل `author_user_id` بلا صنفِ بياناتٍ ولا مسارِ تصديرٍ أو انتهاءِ مدّة. أضف المحادثاتِ الخاصّةَ إلى مشيةِ خروجِ المدرّسِ (`FR-037` من ٠١٣) — ⚠️ **لا تعرفها المشيةُ اليوم**: مدرّسٌ يغادر ورسائلُه الخاصّةُ مع قاصرين في لا مسار
- [X] T177 ⚠️ **ونصُّ المهمّةِ يدّعي أكثرَ ممّا يمكن**: «التثبيتةُ تشمل صنفَ `Archive`» لا سبيلَ إليها هنا — لا صنفَ أرشفةٍ في الوحدة (‏`T175`)، وتعامدُ الأرشفةِ مُقاسٌ في كنسِ ٠١٣ نفسِه (`RetentionSweepIdempotencyTest`) لا هنا. النصُّ الأصليّ: `backend/tests/Feature/Community/CommunityRetentionTest.php` — رسائلُ ومحادثاتٌ وتقييماتٌ وملفُّ كشفٍ تُصدَّر وتُمحى وتنتهي مدّتُها؛ **والتثبيتةُ تشمل صنفَ `Archive`** لأن `Delete` لا تُظهر عطلَ التعامدِ إطلاقاً (‏الصفُّ المحذوفُ لا يعود)
- [X] T178 [P] ✅ **الشرطُ انقلب مرّتَين، والجوابُ النهائيّ: لا تُنشأ.** الكتابةُ تحقّقت في المرحلةِ ٨ (`announcement.updated` · `announcement.hidden`)، لكنّ المرشِّحَ يحرس **قارئاً**، و`Community` لا تقرأ `activity_log` في أيّ ملفّ. صنفٌ يرشّح جدولاً لا يسأله أحدٌ هو بعينِه عطلُ `taxonomy.manage`: مُعلَنٌ ومبذورٌ ومُختبَرٌ ولا يقرؤه سطر. يُنشأ يومَ تُبنى واجهةُ سجلِّ الإعلانات، لا قبله. النصُّ الأصليّ: **الشرطُ تحقّق في المرحلةِ ٨**: `UpdateAnnouncement` و`HideAnnouncement` يكتبان إلى `activity_log` عبر `LogsActivity` (‏`announcement.updated` · `announcement.hidden`)، فلم تعد المهمّةُ شرطيّة. أنشئ `Support/CommunityAuditSubjects.php` — الجدولُ مشترَك، والمرشِّحُ **قائمةُ أصنافٍ** لا حذفُ صفوفٍ بعد الجلب
- [X] T179 [P] وسّع `backend/tests/Feature/Settlement/ContextIsolationTest.php` بمسحِ `Modules/Community/` — الحائطُ الماليُّ يجب أن يُحرَس مفرداتياً كما يُحرَس سلوكياً
- [X] T180 [P] حدّث `docs/README.md`: صفُّ وحدةِ `Community` · النقاطُ · الصلاحيّاتُ الجديدة · معنى «بنودُ المساعدِ صلاحيّاتٌ من شاشةِ الأدوار»
- [X] T181 [P] حدّث `docs/erd.md` بالكياناتِ العشرةِ والتوسيعاتِ الثلاثة **مع طبقةِ كلٍّ منها**
- [X] T182 [P] حدّث `docs/deployment.md`: عمليّةُ `reverb:start` **الدائمةُ الثالثة**، ومشرفُ طابورِ `community`، ومتغيّراتُ البيئة
- [X] T183 [P] حدّث `docs/roadmap.md` بعلامةِ ٠١٠ ✅ وبما خرج من النطاقِ مُعلَناً (‏المجموعاتُ · مرفقاتُ الإعلان)
- [X] T184 [P] أضف إلى `CLAUDE.md` و`AGENTS.md` القواعدَ التي لا تُستنتَج من الكود: الحائطُ عند الفحصِ لا على الدور · `hidden_at` لا `deleted_at` · المعرّفُ على القناةِ سببُه مزدوج · `ReadRanksFor` يقرأ أسبوعاً بعينِه
- [X] T185 شغّل البوّاباتِ الأربعَ خضراء: `php vendor/bin/pest` · `./vendor/bin/pint --test` · `./vendor/bin/phpstan analyse` · `npx tsc --noEmit` · `npm test` — `SC-019`، **بلا `@phpstan-ignore` وبلا baseline جديد**
  - ٢٠٢٦-٠٨-٢٦: `pest` **١٩٢٢/١٩٢٢** (‏٦١٩٨ تأكيداً) · `pint` نظيف · `phpstan` **صفر** · `tsc --noEmit` صفر · `npm test` **٧٣/٧٣** في ١٦ ملفاً. بلا `@phpstan-ignore` واحدة وبلا سطرٍ في baseline.
- [X] T186 ✅ **مُشيت كاملةً ٢٠٢٦-٠٨-٢٦ عبرَ الواجهةِ البرمجيّةِ بأربعِ عمليّاتٍ دائمةٍ حيّة، ووجدت عيبَين لم يرَهما ١٩٢٢ اختباراً**: أسماءٌ **فارغةٌ** في ستِّ شاشات (‏`->with('student:id,uuid,name')` و`name` مشتقٌّ لا عمود)، و`helpful_answer` **غائبٌ عن الكتالوج** فاعتمادُ المدرّسِ لإجابةٍ يمنح صفراً بلا خطأ. الاثنا عشرَ كلُّها خضراءُ بضوابطَ في الاتّجاهَين. **وخطوةُ ٨ قِيست بإيقافِ `reverb` فعلاً**: `201`، الرسالةُ تُقرأ، والفشلُ في `failed_jobs` كـ`MessagePosted → BroadcastException`. ⚠️ وما بقي مؤجّلاً هو **ج-٢ وحدَها** (‏الثانيتان على شبكةٍ حقيقيّة). النصُّ الأصليّ: امشِ `quickstart.md` §ب يدوياً بالاثنتَي عشرةَ خطوة — بما فيها **إيقافُ `reverb`** وإعادةُ لصقِ معرّفِ محادثةٍ ليست للقارئ
- [X] T187 ✅ **الجدولُ قائمٌ منذ ٢٠٢٦-٠٨-٢٤ وج-١ مقيسةٌ بالعين**؛ الأربعُ الباقيةُ مُعلَّقةٌ بسببٍ مكتوبٍ لكلٍّ منها (‏بيئةٌ منشورة · MySQL بحجمٍ حقيقيّ · كتابةٌ متزامنة · مدرّسٌ حقيقيّ). سجّل نتائجَ `quickstart.md` §ج الخمسِ في جدولِ **Deferred Verification** بتواريخِها — بندٌ يبقى «مُعلَّقاً» بلا سببٍ مكتوبٍ هو بندٌ سقط
- [X] T188 ✅ **الستُّ كلُّها في القائمةِ الجانبيّة** (`/messages` · `/reviews` · `/report-cards` · `/manage/grading-schemes` · `/manage/announcements` · `/manage/assistants`)، و`/manage/students/{uuid}/reviews` من `AttendanceSheet`. راجع أن كلَّ شاشةٍ جديدةٍ لها **رابطٌ داخل** من قائمةٍ أو صفحةٍ قائمة — ستُّ شاشاتٍ في هذه المرحلة
- [X] T189 ✅ **ولا خطأً خامّاً**؛ و`.catch(() => undefined)` الأربعةُ في الوحدةِ كلُّها على **اشتراكِ مقبس** لا على جلب — وهو السقوطُ الوحيدُ المسموح، لأن الجدولَ هو المصدر. كُتب السببُ عند الرابعِ الذي كان بلا تعليق. راجع أن كلَّ خطأٍ يمرُّ بـ`fieldErrors()` أو `userMessage()` — **ولا خطأٍ خامٍّ ولا `.catch(() => undefined)`**
- [X] T190 حدّث `specs/010-community-assistants/tasks.md` بعلامةِ الإتمامِ وسجّل أيَّ انحرافٍ عن الخطّةِ في `plan.md` بدل تركِه في رسالةِ التزام
