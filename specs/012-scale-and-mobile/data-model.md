...
starts_at             timestamp
ends_at               timestamp                           -- ⚠️ هذا هو الإغلاق
...
**«مغلقة» تعني `now() >= ends_at` أو `closed_at` مختوم — قراءةٌ لا وظيفة.** لا `->delay()`
...
now ≥ ends_at أو closed_at ⇒ closed     (تُعرَض النتيجةُ، ويُرفَض الانضمام)
```
