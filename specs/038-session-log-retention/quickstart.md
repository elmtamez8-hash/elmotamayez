...
- **`cats` صعدَت ١٩ ← ٢١** — وهذا يُثبِتُ أنَّ **السِّجِلَّ حلَّ المفتاحَينِ إلى مالِكِهما** (`$processed++` لا يقعُ إلّا بعدَ `forCategory()` غيرَ فارغ) — وهو عينُ فحصِ اليتيمِ في `PersonalDataContractCoverageTest`، وهو الثُقبُ الذي كُتِبَتْ هذهِ المواصفةُ من أجلِه.
- **و`find=0` هو الدّليلُ على أنَّ الذّراعَينِ لم ترمِيا** — وليسَ `cats`. ⚠️ **وهذا تصحيحٌ لصياغةٍ كانتْ هنا**: `$processed++` يقعُ **فوقَ** الـ`try`، فـ`cats` كانتْ ستقرأُ ٢١ حتّى لو رمَتِ الذّراعانِ معاً. وما يُثبِتُهُ `find=0` هو **استعلامُ الاختيارِ وحدَه** — ولا مُرشَّحَ واحدٌ، فلا `UPDATE` ولا مقبرةَ ولا `INSERT`. و`RunRetentionSweepJob` يبتلعُ الرميةَ في `findings` بدلَ أن يسقُط، فالصفرُ هنا قياسٌ لا سكوت.
...
2. **وادِّعاءُ أنَّ `retain_days` فارغةٌ في كلِّ الفئاتِ كانَ في نصِّ الطلبِ الأصليّ، ووصلَ المواصفةَ وبقيَ فيها إلى اليوم**: `plan.md` لقطَهُ قبلَ البناءِ، ولكنَّ `spec.md` ظلَّ يحمِلُ النّسخةَ القديمةَ سطرًا كاملاً — وادِّعاءٌ مسحوبٌ يبقى مكتوباً هو ادِّعاءٌ قائم. أُزيلَ الآن.
   ⛔ **والمقامُ كانَ غلطاً أيضاً، وهذا السطرُ نفسُهُ أعادَ اعتمادَهُ بدلَ أن يقيسَه**: كتبَ «`19/35` · فالقياسُ المكتوبُ في الخطّةِ صحيحٌ». المقيسُ على الإنتاج: **٣٦ فئةً بعدَ الفرعِ و٢١ بمدّة** ⇒ ٣٤ و١٩ قبلَه. **فالبسطُ صحيحٌ والمقامُ كانَ غلطاً**، وقد صُحِّحَ في `plan.md`. والدّرسُ أنَّ «الإنتاجُ يؤكِّدُ الرقم» لا تُقالُ إلّا بعدَ قياسِ طرفَيِّ الكسرِ لا طرفٍ واحد.
...
✅ **و`RetentionSweepIdempotencyTest` يؤكِّدُ على `rows_anonymised` الآن** — وسَّعَهُ هذا الفرعُ نفسُه. كانَ تأكيدُهُ يغطّي `rows_deleted` و`rows_archived` وحدَهما، فذراعُ تجهيلٍ لا تتقارَبُ كانتْ **غيرَ مرئيّةٍ لحارسِ التكرارِ نفسِه**. ⚠️ وهذا السطرُ بقيَ مكتوباً بصيغةِ «ما زالَ يجبُ أن يُفعَل» بعدَ أن فُعِلَ — وهو الشكلُ الذي يدفعُ القارئَ لإعادةِ عملٍ منجَز.
