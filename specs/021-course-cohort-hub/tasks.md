...
- [X] T106 [US5] `backend/app/Modules/Learning/Actions/ReadCohortRoster.php` — ⚠️ **بشكلِ `LiveSessions/Actions/ReadSessionRoster.php` القائمِ حرفاً**: `ReadBadgesFor` جماعيّاً، و`avatar_url` من `student_profiles.avatar_path`. ⚠️ و**`first_name` و`last_name`، لا `name`**: `users` بلا عمودٍ بذلك الاسمِ — جلبٌ مُتلهِّفٌ مقيَّدٌ يُسمّي `name` أرجعَ اسماً فارغاً في **ستّةِ** مواضعَ عبرَ أربعِ وحدات.
- [X] T107 [US5] أضِفِ المرتبةَ من `backend/app/Modules/Gamification/Actions/ReadRanksFor.php` — ⚠️ **المفتاحُ يُحذَفُ كلّيّاً لمن لا مرتبةَ له** (FR-051): الغيابُ حالةٌ، و«المركز ٠» رقمٌ يُطبَعُ جنبَ اسمِ طالبٍ أمامَ صفِّه. والمستوى تراكميٌّ من جدولٍ آخَرَ وصحيحٌ اليومَ لمن سجّلَ هذا الصباح.
- [X] T108 [US5] `CohortRosterResource` والمسارُ `GET /cohorts/{cohort}/roster`. ⚠️ **صفرُ حقولِ حضورٍ أو مدّةِ بقاءٍ أو درجةٍ أو ملاحظةِ مدرّس** — تلك أسئلةُ `ATTENDANCE_VIEW`، وهذا المسارُ يفتحُ لكلِّ عضو.
- [X] T109 [P] [US5] `frontend/src/components/courses/tabs/RosterTab.tsx`.
- [X] T110 [P] [US5] ⚠️ `backend/tests/Feature/Learning/CohortRosterExposureTest.php` (SC-012): يمشي **كلَّ حقلٍ** في الحمولةِ ضدَّ قائمةِ سماحٍ — لا حضورَ ولا درجةَ ولا ملاحظة؛ ومَن أُغلِقَتْ عضويّتُه **لا يظهر**.
- [X] T111 [P] [US5] ⚠️ `backend/tests/Feature/Learning/CohortRosterBudgetTest.php` (SC-013): تجهيزان — **٥ ⇄ ٥٠** عضواً — والعددُ هو هو، **مع تأكيدِ حضورِ حقلِ الاسمِ والنياشينِ** لا الكلفةِ وحدَها.
- [X] T112 [P] [US5] اختبارُ `frontend/src/components/courses/tabs/RosterTab.test.tsx`: عضوٌ بلا مرتبةٍ **لا يعرضُ رقماً**.

---

## Phase 8: Polish & Cross-Cutting

- [X] T113 [P] بذرةٌ **إضافيّةٌ فقط** `backend/database/seeders/CohortDemoSeeder.php` — بمفاتيحَ ثابتةٍ و`firstOrCreate`، **غيرُ مسجَّلةٍ في `DatabaseSeeder`** (سابقةُ `StudentDashboardSeeder`). تزرعُ ما تصفُه `quickstart.md`: مجموعتين بمواعيدَ متباعدةٍ · ثالثةً مكتملةً · حصّةً لكلٍّ · ⚠️ **حصّتين غيرَ مُسنَدتين إحداهما لطالبِنا فيها مقعدٌ محجوز** — بغيرِ ذلك الصفِّ لا يستطيعُ أحدٌ رؤيةَ ما إذا كان حجبُ Q3 يبتلعُ مقعداً مدفوعاً.
- [X] T114 [P] مراجعةُ الوصولِ على الصفحةِ كلِّها — تبويباتٌ بلوحةِ المفاتيح، وحالاتٌ مقروءةٌ لقارئِ الشاشة، وحركةٌ تُلغى بـ`prefers-reduced-motion` (NFR-010 · SC-015).
- [X] T115 [P] تحقّقْ من رموزِ السمة: **كلُّ صنفِ لونٍ يُسمّي رمزاً موجوداً في `@theme`**. ⚠️ Tailwind v4 لا يُصدِرُ قاعدةً لرمزٍ غيرِ معرَّف، فيُطلى **لا شيء** بصمت — و`bg-success` **لا وجودَ له**؛ `success` اسمُ نبرةٍ في `TONE_CLASSES` لا رمزُ لونٍ في `@theme`. شحنَ هذا العيبُ ثلاثَ مرّاتٍ ومرّةً باختبارٍ أخضرَ فوقَ علامةٍ غيرِ مطليّة.
- [X] T116 [P] حدِّثْ جداولَ الوحداتِ والمساراتِ والصلاحيّاتِ في `docs/README.md` و`docs/erd.md` بالكياناتِ الخمسةِ والمساراتِ الجديدة.
- [X] T117 [P] أضِفْ إلى `CLAUDE.md` مصائدَ هذه المرحلةِ الثلاثَ: صمّامُ بوّابةِ العضويّة · «السابقة» في مجموعةِ الطالب · حجبُ Q3 لا يمسُّ مقعداً مدفوعاً.
- [X] T118 امشِ فحوصَ `quickstart.md` الثمانيةَ يدويّاً في المتصفّح. ⚠️ **الفحصُ ٢(ب) هو الفحصُ كلُّه**: بناءٌ ينجحُ في (أ) ويفشلُ في (ب) يحبسُ طالباً دفعَ ثمنَ مادّةٍ خلفَ شرطٍ لا فعلَ يُحقِّقُه.
- [X] T119 ⚠️ **اضغطْ كلَّ صفٍّ مقفولٍ في الشجرة** على `http://127.0.0.1:3000/enrollments/{course}` (SC-002)، وتحقّقْ من `frontend/src/components/courses/LessonRow.tsx` أنّ المقفولَ ليس `<Link>` ولا `<button>`. فشلٌ يُقرَأُ نجاحاً: صفٌّ يبدو مقفولاً ويُبحِرُ عندَ الضغط.
- [X] T120 شغِّلِ البوّاباتِ الخضراءَ كلَّها: `php vendor/bin/pest` · `./vendor/bin/pint --test` · `./vendor/bin/phpstan analyse` (بلا خطِّ أساسٍ جديدٍ وبلا `@phpstan-ignore`) · `npx tsc --noEmit` · `npm test`.
- [X] T121 `php artisan migrate` على قاعدةِ البياناتِ المحلّيّة. ⚠️ **لا `migrate:fresh` بلا إذنٍ صريحٍ من المالك.**

**ما مُشِيَ فعلاً في T118** (على القاعدةِ المحلّيّةِ الحيّة، ٢٨ أغسطس، وكلُّ تعديلٍ
أُعيدَ بعدَه):

| الفحص | النتيجة |
|---|---|
| ١ · القفلُ مرئيٌّ قبلَ الضغط | ✅ ثلاثةُ صفوفٍ مقفولةٍ `<div tabindex="-1"` بلا `href` ولا `role` — ضُغِطَتْ كلُّها فلم تُبحِرْ، وكلٌّ يحملُ `code: sequence` وجملةً تُسمّي العنصرَ المطلوب |
| ٢(أ) · شاشةُ الاختيار | ✅ `joinable_exists: true` · كلُّ قفلٍ `no_cohort` · «اختر مجموعتك للبدء» |
| **٢(ب) · الصمّام** | ✅ **الفحصُ كلُّه**: بإغلاقِ المجموعاتِ الثلاثِ وبلا عضويّة — `joinable_exists: false`، ولا `no_cohort` في الشجرةِ إطلاقاً (الأقفالُ الثلاثةُ `sequence` وحدَها)، والجملةُ تقولُ لماذا |
| ٦ · قائمةُ الزملاء | ✅ عضوٌ بمستوى ومرتبةٍ ونيشانَين، وعضوٌ آخَرُ **بلا مفتاحِ مرتبةٍ ولا مستوى** — على بياناتٍ حقيقيّة |
| ٧ · التبويباتُ بلوحةِ المفاتيح | ✅ `ArrowLeft` من «الزملاء» يلتفُّ إلى «المنهج» (منطقيٌّ في RTL)، و`?tab=` يُحدَّثُ ويصمدُ عبرَ التحميل |

⚠️ **والقياسُ وجدَ ما لم تجدْه القراءة**: `«المستوى ٠»` كان يُطبَعُ جنبَ اسمِ طالبٍ
حقيقيٍّ — `ReadRanksFor` كان يقرأُ `level_band` (فئةُ المنافسةِ هذه الفترة، وصفرُها
مشروع) بدلَ `student_progress.level`. والحقلُ يُقرَأُ في **الشاتِ** كذلك، وهو تبويبةٌ
واحدةٌ بعيدٌ عن القائمة. مُصلَحٌ بحالتَين في `RankFallbackTest`.

الفحوصُ ٣ · ٤ · ٥ · ٨ لم تُمشَ يدويّاً — كلٌّ منها يحتاجُ تحويلَ بياناتٍ واسعاً على
القاعدةِ المحلّيّة، وكلٌّ منها مُغطّىً باختبارٍ يقيسُ الشرطَ نفسَه
(`CohortGateSafetyValveTest` · `CohortTransferPreservesEverythingTest` ·
`CohortSessionVisibility` وحالاتُها).
