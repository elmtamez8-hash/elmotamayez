# Implementation Plan: بوابة الدفع وتحصيل الطالب (‏007 `qatar-payments`)

**Branch**: `007-qatar-payments` | **Date**: 2026-08-10 | **Spec**: [`spec.md`](./spec.md)

**Input**: `specs/007-qatar-payments/spec.md` · قرارات المالك في جلسة ‎2026-08-10‎
(‏`Q7`/`Q8`/`Q9`) · الكود المشحون حتى `a44995f`

---

## Summary

**تحويل التحصيل من طابور اعتمادٍ بشري إلى مسارٍ لحظيّ، بلا اختيار بوابةٍ بعينها.**

الطالب يدفع، فيصل إشعارٌ موقَّع إلى خادمنا، فتُضاف الأرصدة ويُرفع الحجب خلال ثوانٍ. وحول
ذلك ثلاثة حرّاس: **تسويةٌ دورية** تلتقط ما ضاع من الإشعارات، **سجلّ تدقيق** لا يُعدَّل،
و**سجلّ تحصيل** للمنصّة وحدها.

**والمقاربة التقنية محكومة بحقيقتين:**

1. **المرحلة توسّع ما شُحن ولا تبني موازياً له.** جدولان جديدان، و**عشر هجرات مرتَّبة** على القائم — وترتيبها ليس تفصيلاً: ثلاثٌ منها تُسقط النشر إن سبقت تنظيفها (`data-model.md` §ط). *(‏كان العدد يُقرأ ‎٩‎ هنا و‎٩‎ في السياق التقني و‎٣‎ في جدول المُخرَجات — ثلاثة أرقام لشيءٍ واحد، مُوحَّدة ‎2026-08-11‎.)*
   وستّة من متطلبات السبيك **منفَّذة سلفاً** فتُثبَّت باختبار ولا تُبنى (‏`research.md` §و).
2. **البوابة مؤجَّلة، والعمل ليس كذلك.** `FR-001`…`FR-036` تُنفَّذ كاملةً فوق عقد المزوّد
   المُوسَّع وتُختبر بمزوّدٍ وهميّ. محوّل البوابة الحقيقية ملفٌّ واحد لاحق لا يلمس منطق
   الطلبات — وهو بالضبط ما يقيسه `NFR-003`.

> ⚠️ **السبيك كُتب في ‎٤‎ أغسطس، قبل شحن ‎006‎ و‎014‎ و‎016‎.** فثلاثةٌ من افتراضاته صارت
> خاطئة وواحدٌ من كياناته صار متعارضاً مع قرارٍ مصدَّق. التفصيل في `research.md` §ب/§ج،
> والتصحيحات مُثبَتة في `spec.md` §Clarifications ‎2026-08-10‎.

---

## Technical Context

**Language/Version**: PHP 8.5 · Laravel 13 · TypeScript 5 / Next.js 15 (App Router)

**Primary Dependencies**: `spatie/laravel-permission` (وضع الفرق، `team_id = workspace_id`)
· `spatie/laravel-medialibrary` (الإيصالات، قرص خاص) · `spatie/laravel-activitylog`
(**مستعمَلة سلفاً** — الافتراض المضادّ في السبيك متقادم) · Horizon/Redis

**Storage**: MySQL في الإنتاج · SQLite للتطوير والاختبار
⚠️ **وهذا فارقٌ يخصّ هذه المرحلة تحديداً**: SQLite يقبل ما ترفضه MySQL الصارمة، ولا حساب
لديها بأعداد بلا إشارة يفيض — فعطلٌ في عرض عمود مبلغٍ **لا يمكن لاختبارٍ محليّ أن يعيد
إنتاجه** (‏`NFR-012`). القاعدة نفسها كلّفت ‎006‎ عمودَي `CAST(... AS SIGNED)`.

**Testing**: Pest · `RefreshDatabase` · `WithWorkspace` · مزوّدٌ وهميّ في `tests/Support/`

**Target Platform**: خدمة ويب — تطبيق Next.js + لوحة Filament على `/admin`

**Project Type**: Web (‏`backend/` + `frontend/`) — وحدة `Payments` القائمة تُوسَّع

**Performance Goals**: `SC-014` — سجلّ تحصيل على ‎١٠٬٠٠٠‎ عملية دون ثانية (p95) بلا مسحٍ
كامل · `SC-001` — من وصول الإشعار إلى رفع الحجب: ثوانٍ

**Constraints**: `QAR` وحدها · وحدات صحيحة صغرى (`NFR-007`) · لا بيان وسيلة دفع في أي
موضع · لا نداء شبكي في اختبار · لا كيان تسوية مدرّس

**Scale/Scope**: ‎٥‎ قصص مستخدم · ‎٣٦‎ متطلباً وظيفياً (‏**٥ منفَّذة و‎١‎ نصفه**) ·
‎١٢‎ غير وظيفي (‏`NFR-013` **مسحوب**) · ‎١٥‎ معيار نجاح · جدولان جديدان · **‎١٠‎ هجرات مرتَّبة** على القائم + هجرتا الجدولين (‏`data-model.md` §ط)

---

## Constitution Check

*بوّابة: تمرّ قبل Phase 0، وتُعاد بعد Phase 1.*

| المبدأ | الحال | كيف |
|---|---|---|
| **I. عزل المستأجرين** | ✅ | كل كيان مصنَّف في `data-model.md` §ي قبل هجرته. `provider_callbacks` مملوك لمساحة عمل بمستأجرٍ **يُحلّ عند المعالجة**؛ و`payment_reconciliation_runs` **مملوك للمنصّة صنف (ب)** — و«بلا مفتاح» في نسختي الأولى وصفٌ للمخطَّط لا طبقةٌ من الثلاث، والدستور يرفض كياناً بلا تصنيف |
| **I — حارس الويب-هوك** | ⚠️ **يحتاج انتباهاً** | مسارٌ عامّ فـ`WorkspaceScope` عديم الأثر تماماً. المستأجر يُشتقّ صراحةً من الطلب المرجعيّ، **ولا يُقرأ من الحمولة** |
| **I — صنف (ب)** | ✅ | `BILLING_AUDIT_VIEW` و`BILLING_COLLECTION_VIEW` منصّيتان + اختبار ‎403‎ لأعلى دورٍ مستأجر. ⚠️ **`billing.*` لا `payments.*`** — الأخيرة عائلةٌ مستأجرة في هذا المستودع |
| **II. المنطق في Actions** | ✅ | `InitiatePayment` · `HandleProviderCallback` · `ReconcilePayments` · `ReversePayment`. القواعد في الـAction لا في التحقّق |
| **III. التكامل بالأحداث** | ✅ | ستّة أحداث جديدة **ومستمعوها مُسمَّون** في `contracts/api.md §٤` — وبلا ذلك كان الدفع الفوريّ ينتهي بعمليةٍ `captured` ورصيدٍ لم يتحرّك |
| **IV. البوابات خضراء** | ✅ | الأربع + فحص التسريب (`SC-012`). **بلا baseline جديد وبلا `@phpstan-ignore`** |
| **V. التفويض بالسياسات** | ✅ | صلاحيتان جديدتان + `PaymentTransactionPolicy` بملكيةٍ عبر `order.user_id` — **لا عضوية مساحة**، وإلا مرّ المدرّس |
| **VI. العقود الظاهرة** | ⚠️ **انحراف معلَن** | `payment_transactions` بلا `uuid` اليوم، وهذه المرحلة أوّل من يكشفه. يُضاف **بثلاث خطوات** (‏`data-model.md` §ط) |
| **VI — شكل الـDTO** | ✅ **بعد تصحيح** | نسختي الأولى كتبت `final readonly class` ترث صنفاً **غير `readonly`** — **خطأ قاتل** في PHP ‎8.2+‎. صُحّح إلى خصائص `readonly` مُرقّاة |
| **IV — المسار الحرج ‎#٨‎** | ✅ **بعد تصحيح** | نسختي الأولى فرضت `redirectUrl` غير قابل للإفراغ على `ManualTransferProvider` — **والتحويل البنكي لا صفحة دفع له** — فتكسر `PaymentTest` والدمج |

**ستّة انحرافات معلَنة، وكلٌّ مُبرَّر في `Complexity Tracking`:** توسيع عقد المزوّد (يأذن
به السبيك نصّاً) · توحيد الوحدات الصغرى عبر **أربعة** جداول · هجرة `courses.price`/`products.price`
· إصلاح سباقَين في كودٍ مشحون · **الويب-هوك بلا `FormRequest`** · **قارئ التدقيق بلا Action**.
*(‏الثلاثة الأخيرة أُضيفت ‎2026-08-11‎ بعد أن كشف الفحص أنها كانت تُنفَّذ ولا تُعلَن — وانحرافٌ
غير معلَن أسوأ من انحرافٍ خاطئ، لأنّ المراجعة لا تراه.)*

⚠️ **وحجّة الانحراف الثاني كانت في نسختي الأولى مبنيّة على اقتباسٍ من `docs/erd.md` لا وجود
له** — وقد صُحّح في `research.md` §د، والحجّة الصحيحة قائمة بذاتها.

**إعادة التقييم بعد مراجعة الوكلاء:** ⚠️ **رسبت النسخة الأولى على ثلاثة مبادئ** —
§VI (شكل الـDTO)، §IV (المسار الحرج الثامن)، §I (تصنيف كيانٍ خارج الطبقات الثلاث) —
وجدولُ التحقّق أعلاه كان يقول ✅ على الثلاثة. **جدولٌ يقيّم نفسه ليس بوّابة**؛ ما وجدها
ستّة وكلاء بأبعادٍ مستقلّة، والتفصيل في `review-findings.md`.

---

## Project Structure

### Documentation (this feature)

```text
specs/007-qatar-payments/
├── plan.md              # هذا الملف
├── research.md          # Phase 0 — القرارات وأسانيدها، والتعارضات المكتشفة
├── data-model.md        # Phase 1 — الكيانات والتصنيف والفهارس
├── quickstart.md        # Phase 1 — تسعة سيناريوهات تحقّق
├── contracts/
│   ├── api.md           # الصلاحيات · المسارات · الحمولات · الأحداث
│   └── provider.md      # عقد المزوّد المُوسَّع والمزوّد الوهميّ
├── checklists/
└── tasks.md             # Phase 2 — لا يُنشئه /speckit-plan
```

### Source Code

```text
backend/app/Modules/Payments/            # ⚠️ الوحدة قائمة — توسيعٌ لا إنشاء
├── Actions/
│   ├── InitiatePayment.php          ★
│   ├── HandleProviderCallback.php   ★  (يتحقّق من المبلغ والعملة — provider.md §د)
│   ├── ReconcilePayments.php        ★
│   ├── ReversePayment.php           ★  (النزاع البنكي — د4)
│   ├── ApproveOrder.php             ⚠️ يُصلَح سباقه
│   ├── RejectOrder.php              ⚠️ **السباق نفسه** — research.md §هـ2
│   ├── UploadPaymentReceipt.php     ⚠️ يُطلق ReceiptUploaded
│   ├── CreateOrder.php              ⚠️ وحدات صغرى
│   └── PurchaseCredits.php          ⚠️ تُحذف minorToDecimal()
├── Contracts/PaymentProviderInterface.php   ⚠️ يُوسَّع (يأذن السبيك)
├── Providers/                       ⚠️ **كان ساقطاً**
│   ├── ManualTransferProvider.php   ⚠️ ينفّذ الدوالّ الثلاث الجديدة — verifySignature() ⇒ false
│   └── PaymentProviderRegistry.php  ★ {provider} ⇒ تنفيذ. مجهولٌ ⇒ 404
├── Data/{ChargeIntent,CallbackEvent}.php    ★ ⚠️ ترث DataTransferObject بخصائص مُرقّاة
├── Enums/{PaymentStatus,PaymentMethod}.php  ★
├── Events/{ReceiptUploaded,ReceiptApproved,ReceiptRejected,
│           PaymentCaptured,PaymentFailed,PaymentReversed}.php  ★
├── Listeners/                       ⚠️ **كان ساقطاً** — بلاه لا يضيف الدفع رصيداً
│   └── ReevaluateOnReversal.php     ★  (والمشحونان يُربطان بـPaymentCaptured)
├── Http/
│   ├── Controllers/{PaymentController,WebhookController}.php   ★
│   ├── Controllers/Admin/{PaymentAuditController,
│   │                      CollectionReportController}.php      ★ ⚠️ Admin/ كالمشحون
│   ├── Requests/                    ⚠️ **كان ساقطاً** — الدستور §II يوجب السلسلة
│   └── Resources/                   ⚠️ **كان ساقطاً** — §VI: كل استجابة عبر Resource
├── Jobs/{ProcessProviderCallbackJob,ReconcilePaymentsJob}.php  ★
├── Models/{ProviderCallback,PaymentReconciliationRun}.php      ★
├── Policies/PaymentTransactionPolicy.php                       ★
├── Support/{BillingAuditSubjects,PaymentFieldAllowlist}.php    ★
├── PaymentsServiceProvider.php      ⚠️ **كان ساقطاً** — Event::listen() ولا EventServiceProvider
├── routes/api.php                   ⚠️ **كان ساقطاً** — الموضع الوحيد للمسارات
└── Database/Migrations/             # ⚠️ M كبيرة — خطؤها يُحمّل صفر هجرة على لينكس

backend/app/Modules/Notifications/Support/NotificationType.php   ⚠️ خمسة أنواع
backend/database/seeders/NotificationTemplateSeeder.php          ⚠️ صفٌّ لكلٍّ — وإلا يُسقَط بصمت
backend/database/factories/Modules/Payments/                     ★ نموذجان جديدان
backend/config/horizon.php · backend/routes/console.php          ⚠️ مشرف وطابور وجدولة

backend/tests/Feature/Payments/          # ١٢ ملفاً ★
backend/tests/Unit/Payments/             # ★ التوقيع · مدى التسوية (NFR-003ب)
backend/tests/Support/FakePaymentProvider.php   ★ ⚠️ في tests/ لا app/

frontend/src/
├── app/(app)/(shell)/billing/pay/       ★ بدء الدفع
├── app/(app)/(shell)/billing/pay/return/ ★ ثلاث حالات: نجاح · فشل · ما زالت معلّقة
├── app/(app)/(shell)/orders/page.tsx    ⚠️ **قائم** — وهو مسار الإيصال الحقيقي
│                                        (‏`billing/receipts/` في نسختي الأولى **لا وجود له**)
├── app/(app)/(shell)/manage/payments/   ★ تدقيق · تحصيل · تسوية — وإلا فـUS4/US5 واجهاتٌ
│                                        لا يصل إليها شيء
│                                        ⚠️ **`manage/` لا `admin/`**: لا وجود لمقطع
│                                        `admin/` في الواجهة إطلاقاً، وكل شاشةٍ إدارية
│                                        شُحنت تعيش تحت `manage/` (billing · settlement ·
│                                        courses · freeze). و`(shell)` بلا `admin` بين
│                                        أبنائه الخمسة عشر
├── app/(app)/(shell)/layout.tsx         ⚠️ **رابطٌ واصل لكل صفحة جديدة** — التنقّل مصفوفة
│                                        `NavItem[]` داخل هذا الملف (‏`:35-74`)، وسطحٌ بلا
│                                        مدخل غير مُسلَّم
├── lib/payments.ts                      ★
└── e2e/payments.spec.ts                 ★
```

**Structure Decision**: توسيعٌ لوحدة `Payments` القائمة. **لا وحدة جديدة** — الطلبات
والإيصالات والأرصدة كلها هنا، ووحدةٌ ثانية للدفع كانت ستحتاج حدثاً لتقول لجارتها إن طلباً
اعتُمد، وهو الحدث الذي يوجد داخلها أصلاً. والفصل الذي يهمّ — عن **التسوية** — قائمٌ ومحروس
ببناءٍ يفشل (`ContextIsolationTest`).

---

## Complexity Tracking

| الانحراف | لماذا يلزم | البديل الأبسط، ولماذا رُفض |
|---|---|---|
| **توسيع `PaymentProviderInterface`** | العقد القائم بلا مدخل لحمولةٍ موقَّعة، و`verify()` تسأل «ما حال ما أعرفه؟» بينما التسوية تسأل «ما الذي نجح عندك ولم أعرفه؟» — وهذا الفرق هو `US2` كلها | إبقاء العقد وبناء التسوية على `verify()`: تمشي المعروف وحده فتبقى **عمياء عن دفعةٍ ضاع إشعارها**، وهي الحالة التي وُجدت لأجلها. والسبيك يأذن بالتوسيع نصّاً |
| **هجرة `orders`/`payment_transactions` إلى وحداتٍ صغرى** | `NFR-007` يمنع العشريّات، وقالب `decimal:2` يعيد **نصّاً** فكل جمعٍ يمرّ بعددٍ عائم. والمرحلة تجعل `payment_transactions` كيانها المركزي | إبقاء العشريّات كما تقول ملاحظة `docs/erd.md`: لكنها كُتبت على فرض وجود بيانات إنتاج، و`PRODUCT.md` يقول **قبل الإطلاق، لا مستخدمين**. فلا بيانات تُهاجَر، وهذه آخر لحظةٍ مجانية. والتأجيل يعني بناء المرحلة فوق النوع الخطأ |
| **هجرة `courses.price` و`products.price`** | `CreateOrder` يكتب `$course->price` في `orders.amount`، فتوحيدٌ نصفيّ يجعل كورس ‎49.99‎ يساوي **‎0.49‎ ريال** — وهو الصنف الذي **لا يعيد اختبارٌ محليّ إنتاجه** | توحيد `orders` وحده: يكسر كل طلب كورس. وترك الاثنين: يبني المرحلة فوق النوع الخطأ ويقارن صحيحاً بنصّ عند كل شحن. **قصّة أساس مستقلّة، وأوّل ما يُقتطع** |
| **إصلاح سباقٍ في كود مشحون** | `isPending()` ثم `update()` تعريف السباق، وصفّان محتجزان لطلبٍ واحد يكسران `FR-032` و`SC-010` مباشرةً — نقدٌ دخل مرّةً يُحسب مرّتين | تركه: `SC-007` يطلب صراحةً صفر أثرٍ مضاعف **بتزامن**، ولا اختبار تزامن على هذا المسار اليوم. والإصلاح فهرسٌ فريد وتحديثٌ شرطيّ ذرّي — أرخص من توثيق العيب |
| ⚠️ **[‏٤‏] الويب-هوك بلا `FormRequest`** (الدستور §II) | التحقّق من التوقيع **يسبق أي فكٍّ للجسد**، و`FormRequest` يفكّ قبل أن يعمل المتحكّم — فالسلسلة الإلزامية هنا تُبطل `FR-005` نفسه. والتوقيع يُحسب على البايتات كما وصلت، وأي فكٍّ وإعادة ترميز يكسره | إبقاء `FormRequest`: يجعل الحمولة مقروءةً قبل التحقّق، وهو نصّ ما يمنعه المتطلّب |
| ⚠️ **[‏٥‏] `PaymentAuditController` بلا Action** (الدستور §II) | سابقةٌ مشحونة حرفياً: `SettlementAuditController` يقرأ عبر `SettlementAuditSubjects` بلا Action. والمُرشِّح **صنفٌ نهائيّ بلا فرع**، فلا قاعدة عملٍ تُلتَفّ عليها من Filament — وهي علّة §II | Action للقراءة: يضيف طبقةً لا تحرس شيئاً، ويخالف التوأم في وحدة التسوية بلا سبب |
| ⚠️ **[‏٦‏] `NFR-008` عُدِّل نصّه** بدل الانحراف عنه | أوجب «قفلاً على السجلّ»، و`lockForUpdate()` **بلا أثر على SQLite** فاختبارٌ عليه ينجح محلياً ولا يثبت شيئاً عن MySQL. النصّ الآن يقبل التحديث الشرطيّ الذرّي، وهو عُرف المستودع المشحون في المقاعد وإعادة الترتيب | تنفيذٌ صامتٌ يخالف النصّ: أسوأ الاثنين — القاعدة تبقى مكتوبةً بخلاف الكود، ومراجعٌ يقرأ السبيك يوقف الدمج بحقّ |

---

## ترتيب التنفيذ

**القصص مستقلّة بالتسليم، والترتيب بالتبعية لا بالجاذبية:**

| # | القصة | يعتمد على | لماذا هنا |
|---|---|---|---|
| **‏0** | الأساس: الهجرات · `PaymentStatus` · العقد المُوسَّع · الوهميّ | — | **لا شيء يُختبر قبله**، والوهميّ شرط `NFR-011` |
| **‏1** | US1 — الدفع الفوري (P1) | 0 | المرحلة كلها في جملة |
| **‏2** | US3 — الإيصال (P3) **قبل** US2 | 0 | **ستّة أسداسه مشحونة**؛ الفجوة أحداثٌ وعنوانُ شبكة وسباق. أرخص تسليم وأعلى قيمة فورية |
| **‏3** | US2 — التسوية (P2) | 1 | تحتاج مساراً تصحّحه |
| **‏4** | US4 — سجلّ التدقيق (P4) | 1 · 2 | يسجّل ما تنتجه ما قبله |
| **‏5** | US5 — سجلّ التحصيل (P5) | كلّها | يعرض ولا ينتج |

⚠️ **‏US3 قبل US2 خلافاً لأولويّتَيهما المعلنتين**، وذلك لأن جرد `research.md` §و غيّر
حجمهما: `US3` صار دلتا ضيّقة على مسارٍ يعمل، و`US2` بناءٌ كامل يحتاج `US1` قبله. الأولوية
تقيس القيمة، والترتيب يقيس التبعية والحجم.

---

## المخاطر

| الخطر | الأثر | الاحتواء |
|---|---|---|
| **الوهميّ يوافق تصميمنا لا بوابةً حقيقية** | يخضرّ كل شيء ثم يسقط أول محوّل | العقد يأخذ **الجسد الخام** لا المُفكَّك، ويحاكي الوهميّ الحالات الخمس بنصّها |
| **سرٌّ أو بيان بطاقة يتسرّب في `payload`** | `SC-012` · مخاطرة تنظيمية | التنقية عند **حدود العقد** لا عند العرض، و`PaymentFieldAllowlist` يفحص القيمة كما يفحص الاسم |
| **الويب-هوك يسبق الكتابة المحلية** | دفعةٌ نجحت وطالبٌ محجوب | `payment_transaction_id` **nullable** عمداً + وظيفة مؤجَّلة + ردّ `202` |
| **التسوية تصحّح ضدّ الطالب** | سحبُ رصيدٍ دُفع ثمنه | `FR-014`: الاتجاه المضرّ يحتاج بشراً — واختبارٌ يفرّق بين الاتجاهين |
| **`finance-admin` غير قابل للإسناد** | لا محاسب مفوَّض | قيدٌ معروف ومُوثَّق؛ السوبر أدمن يعتمد اليوم. الآلية قرارٌ مفتوح للمالك |
| ⚠️ **طابورٌ بلا مشرف Horizon** | `202` أُرسل، والوظيفة **لا يصرّفها أحد** — بصمت | كتلةٌ كاملة في `defaults` **وإدراجٌ في `environments` معاً**. ⚠️ **ونسختي الأولى قالت «في `environments` لا `defaults`» فقلبت نصف الدرس**: `defaults` يوفّر **القيم** المشتركة (`connection` · `queue` · `timeout` · `tries`) و`environments` يقرّر **من يعمل**، والاثنان لازمان — والتعليق المشحون فوق `supervisor-maintenance` يقول ذلك حرفياً. وكتلةٌ في `environments` وحدها بلا `connection` تُسقط تزويد **كل** المشرفين لا مشرف المدفوعات |
| ⚠️ **الويب-هوك على `maintenance`** | أحسّ وظيفةٍ في المنتج (`SC-001`: ثوانٍ) خلف مسحٍ ليليّ بعمليةٍ واحدة و`nice 10` | طابورٌ خاصّ `payments` بمهلةٍ قصيرة؛ والتسوية وحدها على `maintenance` مع `withoutOverlapping()` |
| ⚠️ **بوابةٌ لا يمكن تشغيلها** | `BillingMode::PaymentGateway::isReady()` يعيد `false`، فلا مساحة تُحوَّل إلى النمط الذي تبنيه المرحلة | تُقلَب في هذه المرحلة، **بعد** أن يخضرّ مسار `US1` كاملاً — لا قبله |
| **الهجرة تكسر بيانات تطوير** | إزعاج محليّ | `php artisan migrate` وحدها — **ويُسأل المالك قبل أي `migrate:fresh`** |

---

## ما لا تبنيه هذه المرحلة

محوّل بوابةٍ حقيقية (‏`Q8`) · استردادٌ نقديّ أو `refund()` منفَّذة (‏`Q9`) · محفظة نقدية
(‏`Q4`) · دفترٌ رابع (‏`research.md` §ب) · مقاصةٌ آلية (‏012) · اشتراكاتٌ متكرّرة (‏011) ·
تقسيط · أيّ كيان تسوية مدرّس (‏014، محروس ببناءٍ يفشل).

**وحالة الحافة «مدرّس عليه رصيد سالب» تخرج من النطاق** — تناقض `FR-035` و`Q6` في السبيك
نفسه، ورصيد المدرّس كيان ‎014‎.

---

## Phase 1 مكتملة

| المُخرَج | الحال |
|---|---|
| `research.md` | ✅ ‎٨‎ قرارات بأسانيدها · ‎٣‎ تعارضات · ‎٦‎ متطلبات منفَّذة سلفاً |
| `data-model.md` | ✅ جدولان جديدان · **‎١٠‎ هجرات** · التصنيف الدستوري · الفهارس |
| `contracts/provider.md` | ✅ العقد المُوسَّع · الأنواع · الوهميّ |
| `contracts/api.md` | ✅ الصلاحيات · المسارات · الحمولات · الأحداث |
| `quickstart.md` | ✅ **‎١٢‎** سيناريو (‏١–٩ و‎٩أ‎ و‎٩ب‎ و‎٩ج‎)، أحدها **يجب أن يفشل أوّلاً** |
| `tasks.md` | ✅ **‎١٤٨‎** مهمّة في ثمانية أطوار — Phase 2، وأُضيفت هنا لأنّ الجدول كان يعدّ خمسة مُخرَجات ويسقط السادس |

**تمّ:** مراجعة الخطة بستّة وكلاء (‏`review-findings.md`) · `/speckit-tasks` · مراجعة
المهامّ بستّة وكلاء · `/speckit-analyze`.

**التالي:** `/speckit-implement` من الطور ‎١‎.
