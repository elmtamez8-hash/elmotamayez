...
`PlatformSettings::get('auth.session_limits')` = `['student' => 2]` مفتاحه `platform_role`
— **صفّ في `platform_settings` يحرّره مدير المنصة**، لا قيمة في ملف إعدادات
(research §R16). `config('media.session_limits')` يبقى الافتراضي عند غياب الصفّ.

غير المذكور بلا حدّ: مدرّس يفتح لوحة وجهازين وهاتفاً عمل مشروع، وحدّه يعطّل عمله لا يحميه.

عند الدخول: أنشئ الجلسة **أولاً** ثم أنهِ الأقدم حتى يصل العدد للحدّ. الترتيب مقصود —
FR-023 يمنع رفض الدخول الجديد، فالجديدة تُنشأ دائماً.
...
| العمود | النوع | ملاحظات |
|---|---|---|
| `key` | string(64) **PK** | `auth.session_limits` · … |
| `value` | json | |
