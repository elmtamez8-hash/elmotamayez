...
- [X] T101 [US3] `LeaderboardPerformanceTest.php`: ١٠٠٬٠٠٠ مدخلٍ ⇒ رتبةُ طالبٍ أو أفضلُ عشرين خلال **١٠٠ms (p95)** (`SC-008`) — والقياسُ على الفهرس المركَّب، بلا فرزٍ ولا جمعٍ وقتَ الطلب
...
- [X] T122 [US5] نفّذ `FocusState` (`T036`) وأضِف شرطَ الكتم لطالبٍ في جلسةٍ جارية — 🔴 **لا في `QuietHours`**: تلك تخرج فوراً لغير القنوات الخارجية، **والجرسُ هو السطحُ الوحيدُ الذي يراه طالبٌ يذاكر**، فالوصلُ بها يشحن ميزةً لا تفعل شيئاً. و`isMandatory()` تبقى الصمّامَ (`FR-039`). ⚠️ **وصُحِّح الموضعُ عند التنفيذ**: كُتب هنا «شرطٌ جديدٌ في `DispatchNotification`» وهو لا يعمل — صفُّ الإشعار يُكتب **قبل استشارة أيّ قناة** (‏٠٠٣ FR-007) والجرسُ يقرأ ذلك الصفّ، ففحصٌ هناك إمّا يُسقط الرسالةَ أو يؤجّل تسليماً لا ينظر إليه الجرس. المُنفَّذُ تصفيةٌ على **القراءة** في `NotificationController`
...
- [X] T129 [P] `QueryBudgetTest.php` لـ`GET /gamification/me` و`GET /gamification/leaderboard` — بميزانيةٍ مكتوبةٍ ضدّ **ضِعف** حجم التجهيزة (‏نمط ٠٠٥): ميزانيةٌ ضدّ حجمٍ واحدٍ تمرّ على N+1
- [X] T130 [P] `AwardLatencyTest.php`: زمنُ العملية التي أطلقت المنحَ لا يزيد بقدرٍ يُقاس مع التلعيب مفعَّلاً (`SC-016`)
...
- [X] T135 شغّل البوّاباتِ الأربع: `php vendor/bin/pest` · `./vendor/bin/pint --test` · `./vendor/bin/phpstan analyse` (‏**بلا baseline جديد ولا `@phpstan-ignore`**) · `npx tsc --noEmit` — و`npm test` خامساً (`SC-020`)
