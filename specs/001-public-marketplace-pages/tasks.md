...
- [X] T135 [P] تنزيل الصور واستضافتها ذاتياً في `frontend/public/marketplace/` وتوثيق المصادر والتراخيص في `LICENSES.md` (R10)
  - ✅ **نُفِّذت في ٢٠٢٦-٠٨-١٢ وبقي المربّع فارغاً**: الحجب الأصليّ («Unsplash/Pexels غير متاحين من بيئة البناء») ارتفع في جلسةٍ لاحقة ولم يُحدَّث السطر. أُغلق بالجرد في ٢٠٢٦-٠٨-٢٦.
  - **٢٤ صورة مستضافةٌ ذاتياً**: ثمانٍ في `frontend/public/marketplace/` (‏بطلٌ + سبعةُ بنرات) وستَّ عشرةَ في `backend/database/seeders/assets/` (‏خمسةُ مدرّسين · خمسةُ أغلفة · ستُّ صورِ مراجِعين)، كلُّها WebP بجودة ٨٠ مقصوصةً على مقاسِ الحاوية النهائيّ فلا يقفز التخطيطُ أثناء التحميل. وكلُّها مرجوعٌ إليها فعلاً من `(public)` — لا ملفَّ يتيماً.
  - `LICENSES.md` يحمل لكلِّ ملفٍّ **مصوّراً ورابطَ صفحةٍ أصليّة** يمكن فتحُهما — النسبُ ليس شرطاً في الرخصتَين، لكنّه ما يجعل السطرَ قابلاً للتحقّق. وفيه ثلاثةُ دروسٍ تستحقّ البقاء: **`plus.unsplash.com` ليس Unsplash License** (‏بحثُ Unsplash يخلط المدفوعَ في نفس القائمة والنطاقُ وحدَه هو الدليل، وخمسُ صورٍ من أوّلِ اختيارٍ كانت مدفوعةً واستُبدلت) · وثلاثةُ اختياراتٍ رُفضت لمحتواها لا لجودتها (‏مشهدٌ أوروبيٌّ مُخرَج · أغلفةٌ إنجليزيّةٌ مقروءةٌ على منصّةٍ عربيّة · خلفيّةٌ برتقاليّةٌ هي لوحةُ منافسٍ مرجعيّ) · وترتيبُ صورِ المراجِعين ذكر/أنثى **ليس تزييناً**، فـ`STUDENT_FIRST_NAMES` يتناوب بنفسِه وإعادةُ ترتيبِ إحدى القائمتَين تُعطي أحمدَ صورةَ امرأة.
- [X] T136 [P] إضافة أيقونات SVG من مجموعة مفتوحة المصدر إلى `frontend/src/components/icons/` — بلا CDN وبلا مكتبات مدفوعة
- [X] T137 بذرة الحجم `backend/database/seeders/MarketplaceLoadSeeder.php` (50,000 مدرّس + 10,000 كورس) والتحقق من SC-008 — **نُفِّذت. النتيجة النهائية: 7 من 7 ضمن الميزانية.**
  - القياس الأول أظهر البحث بالاسم عند 1116ms، فعولج بعمود `search_name` غير مطبَّع.
  - **القياس الثاني كشف أن التشخيص الأول كان ناقصاً**: قاعدة مبذورة دفعةً واحدة بلا `ANALYZE` تجعل SQLite يختار الخطط بلا إحصاءات، فتبطؤ **كل** القوائم 3–6 أضعاف. `ANALYZE` واحد (114ms) حوّل «فلتر المادة 2148ms فشل» إلى «588ms نجاح».
  - مع الإحصاءات: `whereHas('user')` = 108ms و`search_name` = 55ms. **العمود تحسين مضاعف، لا إنقاذ.** أُبقي عليه بقرار صريح، وصُحّحت التعليقات كي لا يستشهد أحد بالرقم المضلِّل.
  - أُضيف `ANALYZE` إلى نهاية بذرة الحِمل: قياس قاعدة بلا إحصاءات يقيس شيئاً آخر.
  - **تحفّظ قائم**: الأرقام من SQLite على جهاز تطوير، وتذبذبها بين التشغيلات بلغ الضِعف. الإنتاج MySQL + Redis. **SC-008 يلزمه قياس على MySQL قبل الإطلاق.**
