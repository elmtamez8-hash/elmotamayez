...
| الفعل | المسار | ماذا |
|---|---|---|
| GET | `/billing/balance` | حسابه **الواحد** مقسّماً بسياقاته (`FR-009ج`): لكل مساحة عمل — اسم المدرّس، المشترى، المستهلَك، المتبقّي، الحد، حالة الحجب. **بالأرصدة، بلا مال** (`FR-051` · `FR-021د`) |
| GET | `/billing/transactions` | سجلّه مصفّى بمساحة العمل، مرقّم |
| GET | `/billing/packages?workspace={uuid}` | الحزم المفعَّلة **مسعَّرة لهذا المدرّس**: `total` واحد فقط — **يُمنع** أي مكوّن (`FR-021ج` · `SC-015أ`) |
...
```php
/** سعر التسوية المعتمَد لهذا المدرّس ونوع الحصة في هذه اللحظة، بالوحدة الصغرى. */
public function approvedRateMinor(
    int $workspaceId,
...
`SettlementRate` إلى حمولة. `null` يعني «لا سعر معتمَد» ⇐ **الحزم لا تُعرَض لهذا المدرّس**،
لا سعرٌ افتراضي: بيعُ حصةٍ بسعرٍ لم يعتمده أحد هو الخطأ الذي يُدفع نقداً.

### `Shared\Contracts\AccountStanding` — تنفّذه `Payments`

```php
/** هل هذا الطالب محجوب مالياً في مساحة العمل هذه الآن؟ */
public function isWithheld(User $student, int $workspaceId): bool;
```
