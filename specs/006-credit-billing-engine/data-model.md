...
| `credit_packages` | **مملوك للمنصة — بيانات مرجعية** (v1.2.0) | **لا** | صلاحية الكتابة `BILLING_PACKAGES_MANAGE` المنصّية — راجع §٦ |
...
**والثابتة بين العدّادات تُعلَن**: `remaining = purchased − consumed`.

⚠️ **تصحيح مسجَّل (2026-08-09، عند تنفيذ `US1`)**: الاسترداد **يُنقص** `remaining`
و`purchased` معاً، ولا يمسّ `consumed` أبداً. النصّ الأول هنا قال «يزيد `remaining` ويُنقص
`purchased`» — اتجاهان متعاكسان لا تصدق معهما الثابتة أصلاً، فالمسألة تناقض لا خيار. والطالب
يعيد الأرصدة ويأخذ نقداً (`RefundIssued` تحوّلها 007 إلى مال)، فالأرصدة تخرج والاسترداد عكس
الشراء — وهو نصّ ما تقوله `spec.md:382`: «شراء اعتُمد ثم أُلغي أو استُرد ⇒ قيد `REFUND`
ويُعاد تقييم الحجب»، وإعادة التقييم لا معنى لها إن كان الرصيد يرتفع.

ورفعُ `consumed` بدلاً من ذلك يُبقي الثابتة صحيحة **ويُظهر للطالب حصصاً لم يحضرها** — وهذا
هو الانحراف الذي لا تراه `SC-001`، لأنها تفحص `remaining` وحده.
...
| `workspace_id` | FK | ⚠️ **أُضيف عند التنفيذ 2026-08-08**: §١ يصنّف الجدول «جسراً **بالسمة**» بينما قائمة أعمدته هنا كانت بلا `workspace_id` — والسمة لا تعمل بلا العمود. و`GET /billing/transactions` يقرأ الجدول مباشرةً، فالنطاق العام أصدق من مرشّح يكتبه كل منادٍ بيده. يُسنَد صراحةً كنظيره على الرصيد |
...
| `credit_balance_id` | FK |
| `workspace_id` | FK — ⚠️ أُضيف عند التنفيذ، لنفس سبب `credit_transactions` |
...
⚠️ **وهو الجدول الوحيد بلا `workspace_id`، عمداً** (تصحيح §١ عند التنفيذ): جدول وصلٍ بين صفّين
**كلاهما منطوق عليه أصلاً**، لا يُبلَغ إلا بمعرّفات قيودٍ حلّها المنادي، ولا يقرؤه مسار ولا
تحمله حمولة. نسخةٌ ثالثة من مفتاح المستأجر فيه عمودٌ لا يستعلمه شيء.
...
## ٦ — ✅ التعارض الدستوري حول `credit_packages`: **حُسم بتعديل الدستور**

الدستور v1.1.0 سطر ٥٢ كان يعدّد «**حزم الأرصدة**» ضمن ما تملكه **مساحة العمل**، وسبيك 006
يقول عكسه نصّاً — `FR-016`: «**المنصة** (لا المدرّس) **يجب** أن تعرّف حزم الأرصدة»، وهو
مشتقّ من `Q-1` (التسعير `cost-plus` تملكه المنصة، والمدرّس لا يعرّف سعراً).

**الحسم (2026-08-08)**: `.specify/memory/constitution.md` → **v1.2.0** بإجراء الحوكمة.
والطبقة المنصّية صارت **صنفين**:

| الصنف | مثاله | الحارس |
|---|---|---|
| **(أ) هوية الطالب** وما يتبعه | `student_credit_accounts` · `terms_consents` | ملكية الصفّ للمستخدم + `NFR-001أ` |
| **(ب) بيانات مرجعية منصّية** | `credit_packages` · `platform_settings` | **صلاحية الكتابة المنصّية** — لا مالك فرداً لها |

والصنف (ب) لم يكن اختراعاً لهذه المرحلة: `Tenancy\Models\PlatformSetting` مملوك للمنصة
اليوم بتعليقٍ صريح يقول «deliberately no BelongsToWorkspace»، والنصّ وحده كان يصف الطبقة
بأنها «هوية الطالب وما يتبعه» فبقي خارج أي تصنيف. فالتعديل **دوّن ممارسةً قائمة** ولم يفتح
باباً جديداً.

فجدول `credit_packages` أعلاه: **بلا `BelongsToWorkspace`**، وحارسه `BILLING_PACKAGES_MANAGE`
المنصّية — ويلزمه اختبار يؤكّد أن حاملَ أعلى دور مستأجر يُردّ بـ403 (قاعدة v1.2.0 السارية
الجديدة).
