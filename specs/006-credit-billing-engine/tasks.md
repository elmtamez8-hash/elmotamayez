...
- [X] T009 [P] أنشئ المجلدات الناقصة تحت `backend/app/Modules/Payments/`: `Enums` · `Events` · `Jobs` · `Listeners` · `Support` · `Http/Resources/Manage` — المجلد الأخير هو **نطاق مسح `StudentBalanceAllowlist`** ولا يجوز أن يمسح الوحدة كلها ([api.md §٣ب](./contracts/api.md)) — **محسومةٌ بشرحٍ وتُغلَقُ ٢٠٢٦-٠٩-٢١**: الخمسةُ قائمةٌ ومملوءةٌ (قِيسَ)، والسادسُ لا معنى له كما يقولُ الشرحُ تحتَه. ⚠️ **ومربعٌ يقرأُ خلافَ السطرِ الذي تحتَه يُفقَدُ وقتٌ في إعادةِ التحقيقِ فيه**: حسِبَ عملاً باقياً مرتَينِ في هذهِ الجلسةِ وحدَها — عائلةُ ٠٣٤: المربعُ قارئٌ والقرارُ كاتِبٌ، فَليَتحركْ المربعُ معَ القرار.

> **خمسة من ستة، والسادس لم يعد له معنى.** `Enums` · `Events` · `Jobs` · `Listeners` ·
> `Support` كلها قائمة ومملوءة. أما `Http/Resources/Manage/` فلم يُنشأ لأن **لا Resource
> للوحة أصلاً**: `StudentBalanceController` يبني صفوفه في المتحكّم، فمسحُ مجلدٍ فارغ كان
> سيمرّ دائماً. والحارس الفعلي أقوى مما وُصف — `TeacherPanelRowTest` يقارن **مفاتيح الحمولة
> المُرسَلة** بقائمة السماح وبقائمة المنع، فيفشل على حقلٍ يُضاف أياً كان مصدره.
...
- [X] T097 [P] [US3] ⚠️ **نصفها الخلفي مُنجَز، وواجهتها لا مكان لها بعد**: شاشة *اعتماد* السعر غير موجودة في الواجهة أصلاً — `manage/settlement/page.tsx` شاشة المدرّس التي *يطلب* منها، والاعتماد مسار إداري بلا واجهة. والأهم أن الصياغة الأصلية غير قابلة للتنفيذ كما كُتبت: حقلٌ في حمولة اعتماد السعر يُحتسب من `credit_purchases` **حمولة تسوية تُقرأ من جداول الفوترة**، وهو بالضبط ما يُفشل `ContextIsolationTest`. فالعدّاد صار **مسار فوترة مستقلاً** يناديه المعتمِد: `GET /admin/billing/outstanding?workspace=` بصلاحية `SETTLEMENT_RATE_APPROVE` — سياقان، طلبان، بلا مفتاح بينهما. يبقى ربطه بالشاشة حين تُبنى. النصّ الأصلي: أضف إلى شاشة اعتماد السعر في `frontend/src/app/(app)/(shell)/manage/settlement/` عدّاد **«كم رصيداً قائماً بيع دون السعر الجديد»** — مقروءاً من `credit_purchases`. فرق السعر بين الشراء والتنفيذ **بنيوياً غير قابل للإزالة**، ومخرجه أن يصير خسارةً تُقرَّر لا خسارةً تُكتشَف عند الإقفال
  ✅ **شُحِنَت شاشةُ الاعتمادِ ٢٠٢٦-٠٩-٢١** — `Settlement/Filament/Pages/ReviewRateRequests.php`. ⛔ **والفجوةُ كانت أوسعَ ممّا وصفَه هذا البند**: لم تكنِ الواجهةُ وحدَها ناقصةً، بل **لا سطحَ في المنتَجِ كلِّه يُنادي بابَ القرار** — `‎/admin/settlement/rate-requests/{id}/approve` قائمٌ منذُ ٠١٤، وصفرُ مُنادينَ في `frontend/src` وصفرُ شاشاتٍ في اللوحة (قِيسَ ٢٠٢٦-٠٩-٢١). و٠٢٧ · T083 كانَ قد بنى للمدرّسِ استمارةَ **الطلب** ورفعَ عنه جملةَ «تواصل مع إدارة المنصّة» — فصارَ البابُ مفتوحاً بلا مخرج: يطلبُ ولا أحدَ يستطيعُ أن يُقرِّر. عائلةُ `writeBans.lift` المسجَّلةُ في `CLAUDE.md`. ⚠️ **ولم تكنْ قد عضَّت**: `rate_change_requests` على الإنتاج **صفرُ صفوف** يومَ شُحِنَت الشاشة؛ أوّلُ مدرّسٍ يطلبُ كانَ طلبُه سيقف.
    - ⛔ **والسطرُ الحاملُ ليس الصفحةَ بل اكتشافَها**: `Settlement/Filament/Pages` لم يكنْ مذكوراً في `discoverPages()`، و**مجلَّدٌ غيرُ مذكورٍ لا يُنتِجُ خطأً ولا مساراً** — فصفحةٌ سليمةٌ تماماً تبقى ملفّاً لا شاشة، وتمرُّ `settlement.rate.approve` في كلِّ اختبارٍ لها وهي تحرسُ العدم. والتحذيرُ مكتوبٌ فوقَ ذلك السطرِ **ثلاثَ مرّاتٍ** من ثلاثِ مواصفاتٍ سابقة، وهذه رابعتُها. والحارسُ يقيسُ الاكتشافَ لا المنطق، وقِيسَ بالتحوير: نزعُ السطرِ يُسقِطُ **تلك الحالةَ وحدَها** والثلاثَ الباقياتُ خضراء — وهو عينُ المقصود.
    - ⚠️ **والصلاحيّةُ للسوبر أدمنِ وحدَه، مقيسةً لا مفترَضة**: `finance-admin` ⇐ `false` و`compliance-officer` ⇐ `false` و`super-admin` ⇐ `true`. وهو ما يقولُه `RolePermissionMatrix` بنصِّه («تصلُ إلى السوبر أدمن عبرَ `$all`»)، وتجهيزةٌ بموظَّفٍ ماليٍّ تُخرِجُ الشاشةَ فارغةً وتُقرَأُ عطباً وهي رفضٌ صحيح.
    - ⚠️ **والطابورُ بلا نطاقِ ورشة**: `RateChangeRequest` تحتَ `BelongsToWorkspace`، و`WorkspaceContext::id()` ترتدُّ إلى `users.last_workspace_id` للسوبر أدمنِ أيضاً — فطابورٌ منطوقٌ يعرضُ ورشةَ المُقرِّرِ وحدَها **بلا خطأٍ ولا رسالة**. التجهيزةُ تُعطي المُقرِّرَ ورشةً **غيرَ** ورشةِ الطلب، وهو السطرُ الذي كشفَ العطبَ الخماسيَّ في ٠٢٤.
    - ⛔ **والعدّادُ خارجَ الحمولةِ لم يكنْ كافياً، وقِيسَ بفشلِ البناءِ على الشريحةِ الثالثة**: كُتِبَتِ الشاشةُ أوّلَ مرّةٍ وهي تستوردُ `CreditBalance` و`CreditPurchase` بحجّةِ أنّ العدّادَ «بجوارِ القرارِ لا داخلَ حمولتِه» — و`ContextIsolationTest` لا يسألُ عن الحمولةِ أصلاً في هذه الحالة: يمسحُ **كلَّ ملفٍّ** تحتَ `Modules/Settlement/` ويرفضُ استيرادَ نطاقِ الفوترةِ واسمَ جدولٍ منه مقتبساً، **بأيِّ صورةٍ وفي أيِّ موضع**. فصارَ العدّادُ يصلُ عبرَ عقدٍ مشترَك — `Shared/Contracts/OutstandingCreditsDirectory` تُنفِّذُه الفوترةُ وتسألُه التسوية، وهو عينُ ما وُضِعَ له `SettlementClearance` في الاتّجاهِ المقابل. ⚠️ **والمسحُ لا يُجرِّدُ التعليقاتِ هنا** (بخلافِ مسحِ الحمولةِ فوقَه، وبنصِّ ترويسةِ الملفّ)، فذكرُ ذلك الاستيرادِ نصّاً في شرحٍ يُسقِطُ البناءَ كما يُسقِطُه الاستيرادُ نفسُه — وهو ما أُعيدَت صياغتُه. ⚠️ **و`OutstandingCreditsController` صارَ يقرأُ العقدَ نفسَه**: كانَ الادّعاءُ «القراءةُ نفسُها» نسخاً لاستعلامَين، وهو رقمانِ لسؤالٍ واحدٍ يتفارقانِ عندَ أوّلِ تعديل. وبدونِ العدّادِ يُقرِّرُ المُقرِّرُ وهو لا يعرفُ على كم حصّةٍ مدفوعةٍ سلفاً سيسري قرارُه.
    - ✅ **وأُبلِغَ المدرّسُ بقرارِ سعرِه ٢٠٢٦-٠٩-٢١** — `DecideRateChange::tellTeacher()`. ⛔ **وتصحيحٌ لِما سُجِّلَ هنا قبلَ ساعة**: كُتِبَ أنّ الفعلَ «لا يُبلِّغُ المدرّسَ — لا عندَ الاعتمادِ ولا عندَ الرفض (صفرُ ذكرٍ لـ`DispatchNotification` فيه)». الحقيقةُ الأولى صحيحةٌ والاستنتاجُ منها خاطئ: **الاعتمادُ كانَ يُبلِّغُ منذُ ٠١٤** عبرَ `SettlementRateApproved` → `Settlement/Listeners/NotifyRateDecision`، أي ملفّاً واحداً إلى الجانب. وهي عائلةُ «ادّعاءِ الغياب» المسجَّلةُ في الذاكرة: نفيٌ مقيسٌ في ملفٍّ واحدٍ عُمِّمَ على الشجرة. ⛔ **والناقصُ فعلاً كانَ الرفضَ وحدَه**: `NotificationType::SettlementRateRejected` حالةٌ مُعدَّدةٌ **بقُرّاءٍ بلا كاتب** — صفٌّ في `NotificationCategory`، وقالبٌ مبذورٌ **وحيٌّ على الإنتاجِ منذُ ٢٠٢٦-٠٨-٢٨** (`id 8` · فعّال · `not_required`، قِيسَ) — ولا سطرَ في الشجرةِ كلِّها يُرسِلُه. عائلةُ `ClassSessionStatus::Interrupted`. ⚠️ **فلا قالبَ جديدٌ ولا مايجريشنُ ردمٍ ولا قرارٌ في `targetsGuardians()`** كما قُدِّرَ هنا: الأنواعُ الثلاثةُ تقعُ على `default => false` فقناتُها الجرسُ وحدَه، وعددُ واتساب يبقى ٢٥. ⚠️ **ومن الفعلِ لا من حدث**، وسابقتُه `DecidePlanChange::tellTeacher()` — أقربُ نظيرٍ في المنتَج؛ والاعتمادُ يمرُّ بحدثٍ لأنّ لذلك الحدثِ مستهلِكاً آخرَ (تسعيرُ ٠٠٦)، والرفضُ مستهلِكُه جرسُ المدرّسِ وحدَه. والاسمُ `tellTeacher` لا `notify`: `ProviderAgnosticTest` يُسقِطُ البناءَ على الثاني.
    - ⛔ **وعلاقةُ `teacherProfile()` كانت منطوقةً على الجانبَين، وثالثُها الشاشةُ التي شُحِنَت قبلَها بساعة**: `TeacherProfile` تحتَ `BelongsToWorkspace`، و`WorkspaceContext::id()` ترتدُّ إلى `users.last_workspace_id` لموظَّفِ المنصّةِ أيضاً — فتردُّ العلاقةُ `null` لمن يُقرِّرُ في ورشةِ غيرِه. ثلاثةُ قُرّاء: `NotifyRateDecision` يخرجُ باكراً فلا يُبلَّغُ المدرّسُ باعتمادِ سعرِه، و`ReviewRateRequests` يطبعُ «—» بدلَ الاسمِ في كلِّ صفّ، والرفضُ الجديد. ⚠️ **والتجاوزُ على الاستعلامِ الأمِّ لا يصلُ إلى `with()`** — علاقةٌ محمَّلةٌ مسبقاً تُجري نطاقَها من جديد (قاعدةُ سلسلةِ التدقيقِ في ٠٢٤) — فوُضِعَ على العلاقةِ نفسِها، تهجئةَ `Enrollment::course()`. ⚠️ **ومقيسٌ بالتحوير**: نزعُ التجاوزِ يُسقِطُ الثلاثَ، ونزعُ سطرِ الإبلاغِ يُسقِطُ الرفضَ وحدَه. ⚠️ **ولم يكنْ قد عضَّ على الإنتاج**: `last_workspace_id` للسوبر أدمنِ **NULL** (قِيسَ ٢٠٢٦-٠٩-٢١) و`WorkspaceScope` لا تُضيفُ شرطاً حينَ يكونُ السياقُ فارغاً — فالفخُّ منصوبٌ يقعُ أوّلَ مرّةٍ يفتحُ فيها المالكُ ورشة. و`assertCanSeeTableRecords` تمرُّ فوقَه كاملاً: تسألُ عن وجودِ الصفِّ لا عمّا رُسِمَ فيه.
...
- [X] T111 [US4] طبّق سلوك بلوغ الصفر المضبوط في `CreditLedger` عند نزول الرصيد إلى الصفر: منع الحجز الجديد أو توليد تذكير دفع أو كلاهما، مقروءاً من `BillingSettings` (`FR-027`)

> **تصحيحان أثناء التنفيذ:**
>
> - ~~**`T111` جزئية عمداً.**~~ *(اكتملت: نصفها المُذكِّر هبط مع نوعه وقالبه في `T120`–`T122`.)*
>   نصفها الحاجب مُنفَّذ: `CreditLedger::isBlocked` صار يفصل بين
...
- [X] T112 [P] [US5] اكتب `backend/tests/Feature/Payments/ThresholdNotificationTest.php` ([quickstart §٧](./quickstart.md)): تنبيه داخل المنصة عند الأولى، وتذكير لوليّ الأمر وعدّاد صريح عند الثانية؛ ثم أعد تشغيل الاحتساب ⇒ **صفر** تنبيه جديد؛ ثم ارفع الرصيد وأنزله ⇒ تنبيه واحد جديد (`SC-009` · `FR-034`)
- [X] T113 [P] [US5] اكتب `backend/tests/Feature/Payments/WithholdingTest.php`: طالب تجاوز حدّه في كورس ⇒ الحجز فيه مرفوض **برسالة تبيّن الرصيد المطلوب ومسار الشراء** لا خطأ عام (`FR-032`)، و**كورسه الآخر المسدَّد لا يتأثّر**؛ ثم اعتمد شراءه ⇒ المحاولة **التالية مباشرةً** تمرّ بلا وظيفة ترفع الحجب (`SC-010` · `FR-033`)
- [X] T114 [P] [US5] اكتب `backend/tests/Feature/Payments/TeacherPanelRowTest.php` **على مستوى الصفّ**: طالب مسجَّل عند مدرّس واحد ⇒ لوحة الآخر خالية؛ ثم يُلغى التسجيل ⇒ يختفي الصفّ في الطلب **التالي** (`SC-020` · `FR-055`). قائمة الحقول حارس حقول و`FR-054`/`FR-055` قاعدتا **صفوف**، ومسحُ Resource لا يرى أي صفوف اختيرت. السابقة `PlatformOwnershipTest:99-115`: «الوصول يتبع التسجيل، لا لقطةً أُخذت عند بدايته»
- [X] T115 [P] [US5] اكتب `backend/tests/Feature/Payments/QueryBudgetTest.php` — يقارن **٥٠ صفّ رصيد بـ٥٠٠** ويؤكّد **التساوي** لا سقفاً ثابتاً: تأكيدُ «≤١٥» يمرّ فوق N+1 ما دامت العيّنة صغيرة وهي في الاختبار صغيرة دائماً. و`NFR-012` تُقاس بعدد **صفوف الرصيد** لا الطلاب، وإلا زُرع الاختبار بغير ما ينتجه الإنتاج

### التنفيذ

- [X] T116 [US5] احسب الرتبة من `remaining_credits` وقارنها بـ`notified_tier` **في نفس المعاملة** في `CreditLedger`؛ التنبيه على الانتقال **هبوطاً** وحده، والصعود يصفّر الرتبة
- [X] T117 [US5] أطلق `BalanceThresholdCrossed` و`AccessWithheld`/`AccessRestored` — **ومن المواضع الثلاثة التي تقلب المُسنَد بلا قيد**: `PATCH .../limit` (الحدّ) · `POST /manage/billing/exam-mode` (الأرضية إلى صفر) · `DELETE`/انتهاء النافذة (عودتها). كشفُ التغيّر داخل فعل الرصيد وحده يفوّتها كلها فلا يعلم المحجوب بحجبه حتى الحركة المالية التالية ([events.md §٢أ](./contracts/events.md))
- [X] T118 [US5] أنشئ `backend/app/Modules/Payments/Support/EloquentAccountStanding.php` وسجّله على العقد — `withheldCourseIdsFor` تقرأ الاستحقاق **مرة واحدة لقائمة كاملة**، و**يُمنع** نداء العقد من داخل أي Resource: نداءٌ لكل صفّ هو N+1 بالبناء
- [X] T119 [US5] اربط `backend/app/Modules/LiveSessions/Support/BookingEligibility.php` بـ`AccountStanding` — هو الموضع الذي يُنفَّذ فيه المنع؛ بدونه `SC-010` و`SC-013` بلا تنفيذ لأن العقد كان مُسنَداً إلى `Media` وحدها و`Media` لا تحجز
- [X] T120 [P] [US5] أضف أنواع `NotificationType` الجديدة (عتبة أولى · عتبة ثانية لوليّ الأمر · حجب · رفع حجب) — **والمالي الحرج مصنَّف إلزامياً** (`FR-035`)
- [X] T121 [P] [US5] أضف قوالب الأنواع الجديدة إلى `backend/database/seeders/NotificationTemplateSeeder.php` — **قالب مفقود يعني إشعاراً يُسقَط بصمت** (`TemplateRenderer` يرفض الرندر و`DispatchNotification` يسجّل ولا يفشل)، فكل تأكيد في T112 يمرّ على صفر
- [X] T122 [US5] أنشئ المستمعين للإشعارات — عبر `DispatchNotification` وحدها، و**يُمنع** تسمية قناة في أي ملف تحت `Actions/` (`ProviderAgnosticTest` يُفشل البناء)
...
- [X] T125 [P] [US5] أنشئ `frontend/src/app/(app)/(shell)/manage/billing/page.tsx` — لوحة المدرّس بالأرصدة وحالة الحجب، **بلا مال**، وأضف رابطها إلى تنقّل اللوحة
- [X] T126 [P] [US5] أنشئ شاشة الطالب المحجوب: السبب والمطلوب ومسار السداد لا رسالة خطأ عامة (`US5/7`)
- [X] T127 [US5] أنشئ `GET /api/v1/billing/balance?student={uuid}` لوليّ الأمر عبر `GuardianDirectory::childrenOf(..., GuardianPermission::Payments)` — **الصلاحية تحديداً**: «وليّ أمر لا يجوز له رؤية السجلّ المالي لا شأن له بتذكير دفع عنه»

> **ثلاثة أثناء التنفيذ:**
>
> - **`T117` جزئية بحكم الترتيب لا بحكم قرار.** ~~موضع الحركة المالية مُنفَّذ بالكامل في~~
>   *(محدَّث بعد `US6`: بقي موضع واحد — `T148`.)* موضع الحركة المالية مُنفَّذ بالكامل في
>   `BalanceAnnouncer`، والموضعان الآخران **لا وجود لنقطتيهما بعد**: `PATCH .../limit` هي
>   `T134` في `US6`، و`POST · DELETE /manage/billing/exam-mode` هي `T148` في `US8` — وكلتاهما
>   تنصّ صراحةً على الإطلاق من عندها. الحارس أن `announce()` يقيس المُسنَد قبل الكتابة وبعدها،
>   فأي كاتب جديد يستدعيه يحصل على الانقلاب بلا نسخة ثانية من الشرط.
> - **`AccountStanding` كسبت `creditsNeededFor()`.** `FR-032` تشترط أن تبيّن الرسالة **الرقم**
>   ومسار الشراء، ورافضٌ خارج Payments لا يستطيع حسابه بلا الوصول إلى رصيدٍ لا يجوز له معرفته.
>   الرقم وحده يعبر الحدّ، وكلّ رافض يصوغ جملته حوله.
> - **`T115` كشفت N+1 حقيقية.** `WithholdingReader` كان يقرأ `$balance->workspace` داخل الحلقة
>   — ٢٦ استعلاماً لأربعين صفّاً مقابل ٩ لأربعة. صارت المساحات تُجلَب دفعةً واحدة وتُمرَّر
>   إلى الصيغتين الصريحتين للمُسنَد؛ وهذا هو سبب وجود الصيغتين أصلاً.
...
- [X] T128 [P] [US6] اكتب `backend/tests/Feature/Payments/CreditLimitTest.php`: الارتفاع بعد ثلاث عمليات في موعدها بـ+١ حتى سقف ٤، والانخفاض إلى صفر مع التحويل إلى الدفع المسبق بعد ١٤ يوماً برصيد سالب — والقيم مقروءة من `platform_settings` لا مثبَّتة (`SC-012` · `FR-037`)
- [X] T129 [P] [US6] أضف حالة `FR-014`: في `prepaid_credits` **يُمنع** أن يهبط الرصيد تحت الصفر إطلاقاً بصرف النظر عن أي حد
- [X] T130 [P] [US6] أضف حالة `FR-038`: مدرّس يحاول تعديل الحد ⇒ **403**؛ والاستثناء المسجَّل بمن نفّذه ومتى وسببه يمرّ بصلاحية المنصة وحدها

### التنفيذ

- [X] T131 [US6] أنشئ `backend/app/Modules/Payments/Actions/EvaluateCreditLimit.php` — القواعد من `BillingSettings`/`platform_settings` بمفاتيح `billing.limit.*`، والسقف الأقصى يُفرَض **داخل الـAction**
- [X] T132 [US6] اكتب `negative_since` عند النزول تحت الصفر وامحُه عند العودة — بدونه يشتقّ كنسُ الحدّ «كم يوماً سالباً» بمسح سجلٍّ لكل رصيد
- [X] T133 [US6] أنشئ `backend/app/Modules/Payments/Jobs/EvaluateCreditLimitsJob.php` مجدولة بـ`forWorkspace()`، تقود من الفهرس `(workspace_id, negative_since)`
...
- [X] T135 [US6] سجّل تاريخ الحد في `activity_log` بـ`spatie/activitylog` — القيمة السابقة والجديدة والسبب، وهو نصّ `FR-039`. **لا جدول `credit_limit_changes`**: نسخة ثالثة من الفكرة نفسها
- [X] T136 [US6] نفّذ التحويل التلقائي إلى الدفع المسبق للمتعثّر (`FR-040`) وأطلق منه `AccessWithheld` عند انقلاب المُسنَد

> **أربعة أثناء التنفيذ:**
>
> - **ثغرة في `isBlocked` كشفها `T136` نفسه.** فرع «مؤجَّل بلا رصيد» كان `floor < 0`، وهو
>   وكيلٌ عن «عليه دَين». والتحويل إلى الدفع المسبق ينزل بالسقف إلى صفر **والرصيد سالب كما
>   هو** ⇒ الأرضية صفر ⇒ في مساحة عمل على `remind` يواصل المتعثّر الحجز عند −٣ و−٤ إلى ما لا
>   نهاية، وهو الطالب الذي وُضع التحويل من أجله بالذات. صار الشرط يسأل الرصيد أيضاً
>   (`$floor < 0 || remaining < 0`)، ويبقى `remind` لحالته الأصلية: نفاد عند الصفر بلا تأجيل.
> - **`on_time_payments` عمود جديد، ولا يُشتقّ.** «سدّد في موعده» حقيقةٌ عن **لحظة** وصول
>   الدفعة، والدفعة نفسها تمحو `negative_since` — فبعدها بدقيقة يبدو المتأخّر عشرين يوماً
>   نظيفاً في السجلّ. سابقته على الجدول نفسه: `notified_tier` و`negative_since`، وكلاهما حالة
>   مشتقّة خُزِّنت لأن الاشتقاق بعد الواقعة إما مستحيل أو مسحٌ للسجلّ كلّه.
> - **`SetCreditLimit` هو الكاتب الوحيد.** السقف والموافقة (`FR-048`) وسطر التدقيق
>   (`FR-039`) وإعلان الانقلاب أشياء تلزم الطريقين معاً — القاعدة الآلية واستثناء المنصة —
>   وتركها في كلٍّ منهما يعني أن الثاني كتابةً هو الذي ينقصه سطر التدقيق.
> - ~~**المنحة الابتدائية `0 → 1` مؤجَّلة إلى `US9`.**~~ *(نُفِّذت في `T153`/`T155`، ومنحُها
>   حدثٌ عند أول موافقة لا حالةٌ تُشتقّ — التفصيل في ملاحظات `US9`.)* `FR-048` تجعل الموافقة الموثَّقة
>   بوّابتها، والفعل الذي **يسجّل** الموافقة هو `T15x` ولم يُكتب بعد؛ ومنحُ سقفٍ من مجرّد غياب
>   الرفض عكسُ ما تنصّ عليه. ما شُحن هنا هو **الحركة**: الارتفاع والانخفاض واستثناء المنصة —
>   وكلّها تسأل `DeferralConsent` وترفض بلا موافقة. وموضع `T117` الثاني (`PATCH .../limit`)
>   صار موجوداً ويطلق عبر `BalanceAnnouncer::announceStandingChange()`؛ يبقى `T148` وحده.
...
- [X] T137 [P] [US7] اكتب `backend/tests/Feature/Payments/HighValueAssetTest.php` (`SC-011`): الأصل المصنَّف مرفوض بسببه والمطلوب، والعادي يُفتح، و**أصل الكورس المسدَّد يُفتح** — الحجب بالكورس لا بالشخص
- [X] T138 [P] [US7] أضف حالة `FR-043`: الفحص يقع عند **كل إصدار وصول** لا مرة واحدة — أصدر منحة ثم أنزل الرصيد ثم اطلب منحة ثانية ⇒ مرفوضة
- [X] T139 [P] [US7] أضف حالة `FR-044`: عودة الرصيد موجباً تفتح الوصول **في المحاولة التالية مباشرةً** بلا وظيفة

### التنفيذ

- [X] T140 [US7] أضف `is_high_value` إلى تحرير الدرس وموارده في `backend/app/Modules/Courses/` — المدرّس يصنّف محتواه (`FR-041`)
- [X] T141 [US7] اربط إصدار منحة التشغيل في `backend/app/Modules/Media/` بـ`AccountStanding::isWithheld($student, $courseId)` — الفحص عند **الإصدار** حيث بُني الحارس في 004
- [X] T142 [US7] استعمل `withheldCourseIdsFor()` في أي قائمة أصول — **قراءة واحدة ثم ترشيح في الذاكرة**، وأضف حالة إلى مسح `Manage/` تمنع نداء العقد من داخل Resource

> **ثلاثة أثناء التنفيذ:**
>
> - **الرفض حالة ٤٠٢ برمز `access_withheld`، لا ٤٠٣.** الطالب **مستحقّ** — الدرس درسه وحصصه
>   مفتوحة — والناقص دفعة. و«لا تملك صلاحية» جملةٌ كُتبت للغريب، فرفضٌ بها يترك الطالب أمام
>   جدار يعبره بدفعة واحدة بلا أن يعرف. الاستثناء `AccessWithheldException` يحمل **العدد**
>   (من `AccountStanding::creditsNeededFor()` — وهو سبب وجودها على العقد) و**uuid الكورس**،
>   ويُلتقط **قبل** `RuntimeException` الذي يرثه وإلا سُطِّح إلى ٤٠٣ عارية.
> - **الفحص بعد كل فروع الاستحقاق، لا داخل أحدها.** تسجيل المراجعة أصلٌ مصنَّف **وتسجيل
>   حصة** معاً، وفرعه يعود بجواب المقعد قبل النظر إلى التصنيف — فلو عُلِّق المنع على فرع
>   الدروس العادية وحده لعبر أغلى ما في القائمة. وصاحب المحتوى ومدير الحصص لا يحتاجان
>   استثناءً: لا رصيد لهما في الكورس، و**رصيدٌ غير موجود ليس محجوباً**.
> - **التجديد خارج النطاق عمداً.** `RenewPlaybackGrant` لا يعيد فحص التسجيل أصلاً؛ يحدّ عدد
>   التجديدات ويحفظ الموضع. و`T138` تعرّف المطلوب بأنه **رفض الإصدار التالي** لا قطع مشاهدة
>   جارية — وهو شكل «بلا إبطال بأثر رجعي» نفسه في `US8`. فالملف المفتوح يُكمَل، والمنحة
>   التالية تُرفض.
...
- [X] T143 [P] [US8] اكتب `backend/tests/Feature/Payments/ExamModeTest.php` (`SC-013`): صفر حجز برصيد غير كافٍ **مهما كان الحد**، وحجز قائم قبل التفعيل **لا يبطل** (`FR-047`)، وانتهاء الفترة يعيد النمط تلقائياً
- [X] T144 [P] [US8] أضف حالة الحدود الزمنية: نافذة تنتهي اليوم ⇒ الحجز بعد منتصف الليل **غير مقيَّد** — الحدّ الأعلى لمقارنة timestamp بتاريخ هو **بداية اليوم التالي**، و`<= ends_on` يُسقط كل ما بعد منتصف ليل اليوم الأخير

### التنفيذ

- [X] T145 [US8] أنشئ `backend/app/Modules/Payments/Actions/ManageExamModeWindow.php` ونموذج `ExamModeWindow` بـ`BelongsToWorkspace`
- [X] T146 [US8] اقرأ التغطية بمقارنة **نصّ تاريخ** لا بـ`whereDate()` — الدالة حول العمود تُلغي الفهرس `(workspace_id, starts_on, ends_on)`، وهو الفهرس نفسه الذي كلّف `FreezePeriod::covering()` إصلاحاً في 005
- [X] T147 [US8] أجبر `CreditLedger::floor()` إلى صفر داخل النافذة — من `BillingSettings`، بلا فرع ثانٍ للقرار
- [X] T148 [US8] أنشئ `POST · DELETE /api/v1/manage/billing/exam-mode` بصلاحية `BILLING_EXAM_MODE_MANAGE` وبـ`throttle:billing`، وأطلق منهما `AccessWithheld`/`AccessRestored` (T117)
- [X] T149 [P] [US8] أنشئ شاشة نافذة الامتحانات في لوحة المدرّس

> **أربعة أثناء التنفيذ:**
>
> - **`T144` كشفت عيباً حقيقياً في كود 005 المشحون، لا في كود هذه المرحلة.** العمود `DATE`،
>   لكن Eloquent يكتب خاصيةً بـ`date` cast عبر صيغة **التاريخ والوقت** للنموذج، فالمخزَّن
>   `2026-08-09 00:00:00`. MySQL يبترها عند الإدراج فيصحّ `starts_on <= '2026-08-09'`؛ وSQLite
>   يبقيها نصّاً و`'2026-08-09 00:00:00' <= '2026-08-09'` **خطأ** — فالفترة **لا تغطّي يومها
>   الأول**. و`FreezePeriod::covering()` تحمل العيب نفسه حرفاً بحرف: كل اختبارات 005 سألت عن
>   يومٍ **في وسط** الفترة فمرّت. أُصلح الاثنان بـ`starts_on < بداية اليوم التالي` — وهي
>   القاعدة التي تنصّ عليها `T144` نفسها — مع حالة انحدار في `FreezePeriodTest`.
> - **أُضيف `GET /manage/billing/exam-mode` وليس في القائمة.** الشاشة لا تعرض حالةً لا تملك
>   طريقاً لقراءتها، ومفتاحٌ لا يستطيع الإبلاغ عن وضعه مفتاحٌ يُقلَب مرّتين.
> - **`DELETE` بلا uuid عمداً.** سؤال المدرّس «أطفئ الوضع»، وإغلاق صفٍّ واحد يترك نافذةً
>   ثانية متداخلة سارية خلف شاشةٍ تقول إنه مطفأ.
> - **الإعلان بصيغة جماعية جديدة** (`standingsFor` + `announceStandingChanges`): هذا الكاتب
>   يقلب مساحة عمل كاملة دفعةً واحدة، والصيغة المفردة تحلّ النمط والنافذة **لكل طالب** —
>   استعلامان لكل صفّ على شاشة تُفتح مرّتين في السنة. والحالة **تُقاس قبل الكتابة وتُمرَّر**،
>   وتُقرأ الأرصدة من جديد بعدها: `stamp()` يكتب على النموذج الذي يُعطاه، فإعادة وسمه على
>   نفس النسخ تمحو الأجوبة التي تُقارَن بها فيقرأ كل انتقال «بلا تغيير».
...
- [X] T150 [P] [US9] اكتب `backend/tests/Feature/Payments/TermsConsentTest.php` (`SC-014`): صفر تفعيل للتأجيل بلا موافقة مسجَّلة؛ ونسخة شروط جديدة **يُمنع** نسبة الموافقة السابقة إليها (`FR-049`)؛ وموافقة التأجيل **لا تُغني** عن موافقة معالجة البيانات ولا العكس (`FR-050`)
- [X] T151 [P] [US9] أضف حالة التفويض: مستخدم يوقّع باسم طالب ليس ابنه ⇒ **مرفوض** — وإلا وقّع أي مستخدم وثيقةً قانونية باسم غيره، **وأكّدت الاستجابة أن المعرّف لشخص حقيقي**

### التنفيذ

- [X] T152 [US9] هيّئ `TrustProxies` في `backend/bootstrap/app.php` **قبل** الاعتماد على `$request->ip()` — غير مهيّأ اليوم، فالعنوان المسجَّل في الإنتاج هو عنوان موازِن الحمل أي **العنوان نفسه للجميع**، في السجلّ الذي وُجد للاحتجاج به
- [X] T153 [US9] أنشئ `backend/app/Modules/Payments/Actions/RecordTermsConsent.php` — يفحص أن الموقِّع هو الطالب نفسه أو `GuardianDirectory::isAuthorised(..., GuardianPermission::Payments)` **قبل** الكتابة، ويحفظ الوقت والـIP و`user_agent` ومعرّف نسخة الشروط
- [X] T154 [US9] أنشئ `POST /api/v1/billing/consents` بـ`throttle:billing` (**لا `throttle:auth`** — T005) وشاشة القبول
- [X] T155 [US9] امنع تفعيل نمط يسمح بالتأجيل لطالب بلا موافقة سارية في `CreditLedger`/`EvaluateCreditLimit` (`FR-048`) — والسقف الابتدائي **صفر** بلا موافقة موثّقة و**١** بعدها (`Q-9`)
- [X] T156 [US9] استثنِ `terms_consents` من محو 013 بوصفه سجلّ التزام قانوني، واكتب مدّة حفظه في `docs/README.md`

> **ستة أثناء التنفيذ:**
>
> - **الموافقة صارت تُسأل في الأرضية نفسها (`CreditLedger`)، لا عند الكاتبين وحدهما.**
>   `T155` تسمّي `CreditLedger` ولم يكن فيها شيء يسأل: الحارسان كانا في `SetCreditLimit`
>   و`EvaluateCreditLimit`، وكلاهما يحرس **لحظة الكتابة**. فطالبٌ وافق على النسخة ١ ونال
>   سقفاً، ثم نُشرت النسخة ٢ ⇒ الموافقة لم تعد سارية بينما السقف مكتوب والأرضية `−١` ⇒
>   التأجيل يعمل على موافقةٍ ميتة، وهو نصّ ما تمنعه `FR-049`. الآن `floorForBalance()` تسأل،
>   و`floorFor()` تأخذ الجواب معاملاً **افتراضه `false`**: من ينسى يحصل على أرضية صفر — رفضٌ
>   ظاهر — لا على دَينٍ يجري صامتاً. والسقف **لا يُصفَّر** عند النشر بل **يُهمَل**، فيعود
>   ساري المفعول في الحجز التالي لتوقيع النسخة الجديدة، بلا كنسٍ ذاهبٍ ولا كنسٍ عائد.
> - **قاعدة تركيب السقف الابتدائي: `min` لا `max`، والسبب تعارضٌ داخل السبيك.** سطر ١٨٧
>   يكتب الصيغة بالدورة، والفقرة التي قبله مباشرةً **تحرّم** أن تشتقّ الدورةُ السقف —
>   و`BillingSettings` كان يوثّق `max(initialLimitCredits, cadenceAllows)`، وهو تحت دورة
>   شهرية يفتح على **٤** بينما `Q-9` تقول **١** ويسلّم السلّم كلّه في اليوم الأول.
>   `initialLimitFor()` تحسم بـ`min`: الدورة **تخفض** ولا **تُنتج**، فتصدق القاعدتان معاً.
>   وعُدِّل توثيق `BillingSettings` في التغيير نفسه، وإلا شُحن الكود ومعلّقه متناقضين.
> - **المنحة الابتدائية حدثٌ يقع مرّتين لا حالةٌ تُشتقّ.** تُمنَح عند **أول** موافقة على
>   وثيقة التأجيل (`RecordTermsConsent`)، وعند **ميلاد** أي رصيد يُفتَح بعدها
>   (`CreditAccounts::balanceFor()`) — وإلا جلس الطالب المسجَّل عند معلّم ثانٍ الشهر القادم
>   على صفر حتى تصنع له ثلاث دفعات سقفاً من العدم. اشتقاقها في `EvaluateCreditLimit`
>   (`limit === 0 && consent`) كان يعيد لمن حوّلته `FR-040` سقفَه عند أول سداد، فتدوم
>   العقوبة كنسةً واحدة. ولذلك «أول موافقة» تُسأل بـ`everAccepted` لا بـ`has` المرتبطة
>   بالنسخة: تحت الثانية **كل نشرٍ جديد أوّلُ موافقة**، وتعود العقوبة الملغاة مع كل نسخة.
> - **`TrustProxies` في `AppServiceProvider::boot()` لا في `bootstrap/app.php`.** `T152`
>   تسمّي الملف، لكن مغلّف `withMiddleware` يُنفَّذ والتطبيق **قيد البناء** ومستودع الإعدادات
>   غير مربوط بعد — `config()` هناك ترمي `Target class [config] does not exist`، و`env()`
>   ليست بديلاً لأن الإعدادات المخبّأة تعني أن `.env` لا يُحمَّل أصلاً. والافتراض **عدم
>   الثقة بأحد**، لا `'*'`: النجمة تجعل `X-Forwarded-For` من اختيار العميل، فيكتب أيّ أحدٍ
>   أيَّ عنوان في السجلّ الذي وُجد ليُحتجّ به — عنوانٌ مزوَّر أسوأ من عنوانٍ خاطئ صادق.
> - **أربعة ملفات اختبار مشحونة تغيّرت، وهي إفادةٌ عن سلوك لا تنظيفٌ للون.** تشديد
>   الأرضية جعل كل تجهيزةٍ تصف **سقفاً مكتسَباً** ناقصةً بلا موافقة: `ExamModeTest`
>   و`BillingModeTest` و`ConcurrentConsumeTest` أضافت موافقة في `beforeEach` (وإلا مرّت
>   دعاواها لأن السقف صار صفراً، أي **للسبب الخاطئ**)، و`BalanceFloorTest` صار يمرّر
>   `consentCurrent:` صراحةً **ويؤكّد اتجاه الرفض الافتراضي** في حالة مستقلّة. ولا اختبار
>   واحد رُخِّص: الست حالات التي سقطت كانت الست التي تنبّأ بها التشديد بالضبط.
>   **ولازمة**: كل مسار يصل إلى `SetCreditLimit` بسقفٍ موجب من داخل `DB::afterCommit`
>   يسأل `has()` قبله — `EvaluateCreditLimit` تسأل قبل الرفع، والمنحة الابتدائية تقع بعد
>   تسجيل الموافقة مباشرةً — وإلا كان الرفض استثناءً غير ملتقَط فوق دفعةٍ حُصّلت فعلاً.
> - **`GET /billing/consents` وشاشة القبول قسمٌ في `/billing` لا صفحة.** الـGET خارج
>   القائمة، بنفس حجّة `T148`: لا يمكن لشاشة أن تطلب موافقةً لا تعرف أنها مطلوبة، والنسخة
>   السارية هي بالضبط الشيء الذي يتغيّر. والقسم يختفي حين لا شيء معلّق ويعود وحده عند نشر
>   نسخة جديدة. **وشاشة وليّ الأمر مؤجَّلة**: المسار موجود ومختبَر في `T151` (`student` في
>   الجسم)، لكن الواجهة تُوقّع عن النفس فقط. **وملاحظة**: نشر نسخة جديدة من اللوحة يقلب
>   أرضيات الجميع بلا مُعلِن يعمل — لا مهمّة في القائمة تغطّي مسار النشر، فلم يُبنَ.
...
- [X] T157 أنشئ `backend/app/Modules/Payments/Jobs/ReconcileCreditBalancesJob.php` مجدولة تكتب الانحرافات في جدول نتائج — **المطابقة وظيفةٌ لا صفحة**: كما وُصفت أولاً هي `GROUP BY` على أسرع جداول المرحلة نمواً بلا مرشّح مستأجر ولا ترقيم على GET
- [X] T158 أضف إلى الوظيفة الثابتتين اللتين تكشفان ما تعمى عنه المطابقة البسيطة — طرفاها يكتبهما نفس المسار في نفس المعاملة فحصةٌ لم تُشحَن أصلاً تتركهما متطابقين: `الحصص المُسلَّمة × مقاعدها == قيود الاستهلاك` و`SUM(credit_lots.credits_remaining) == remaining_credits` حين يكون موجباً
- [X] T159 أنشئ `GET /api/v1/admin/billing/reconciliation` يقرأ جدول النتائج **مع وقت آخر تشغيل** بصلاحية `BILLING_PRICING_MANAGE`
- [X] T160 [P] أنشئ `ExpireCreditLotsJob` **مطفأةً** واربطها بحدث `CreditExpired` القائم من T029 (هذه المهمة الوظيفة والتوصيل وحدهما، لا الحدث): `expires_at` افتراضه `null` أي «لا تنتهي»، وترتيب «الأقرب انتهاءً أولاً» قائم من اليوم الأول — تفعيله لاحقاً هجرةٌ على أرصدة اشتراها الناس على أنها دائمة (`Q-5`)
- [X] T161 [P] أنشئ كنس الخمول (`Q-8`): بعد `billing.dormant_notice_months` بلا نشاط على `last_transaction_at` يصل الطالب **إشعار** بما لديه ومسار الاسترداد — تذكيرٌ لا انتهاء، ولا مصادرة ولا نقل إلى مدرّس آخر (النقل إعادة تسعير)
- [X] T162 [P] أضف بذرة أرصدة إلى `backend/database/seeders/ScenarioSeeder.php`: طالب برصيد موجب وآخر سالب محجوب وثالث عند عتبة تنبيه، وحزم مفعَّلة، ونافذة امتحانات — كي لا تكون الشاشة فارغة عند أول فتح
- [X] T163 [P] حدّث `docs/README.md` بجداول المرحلة ونقاط النهاية والصلاحيات الثماني الجديدة، وبمدّة حفظ `terms_consents`
- [X] T164 [P] حدّث `docs/erd.md` بالجداول التسعة وطبقات ملكيتها الثلاث، **وارسم غياب** أي مفتاح بينها وبين جداول التسوية صراحةً
- [X] T165 [P] حدّث `CLAUDE.md` و`AGENTS.md` معاً بمزالق هذه المرحلة: `insertOrIgnore` تكسر `HasUuid` · الحساب الموقَّع وفخّ `1690` · الترتيب إدراجاً قبل خصماً · الأرضية تحرس الحجز لا التسليم · اعتماد شراء الأرصدة صلاحية منصة · `Queue::fake()` الجزئي
- [X] T166 [P] حدّث `docs/roadmap.md` بحالة 006 وأثرها على 007 و009 و011 و015
- [X] T167 راجع كل مسار كتابة: محدود المعدّل بمحدِّد **مسمّى** لا سطري · وكل مسار عرض مالي محروس بصلاحية صريحة (`NFR-013`)
- [X] T168 شغّل البوابات الأربع: `php vendor/bin/pest` · `./vendor/bin/pint --test` · `./vendor/bin/phpstan analyse` (Larastan L8 **بلا baseline جديد**) · `npx tsc --noEmit` (`SC-021`)
- [X] T169 شغّل `npx playwright test` على **بناء إنتاج** مع `PHP_CLI_SERVER_WORKERS=8 php artisan serve` — الخادم أحادي الخيط يرفض طلبات ما قبل التصيير المتوازية فيسقط البناء قبل أول اختبار. وأوقف خادم التطوير أولاً (**بإذن المستخدم**): كلاهما يكتب `.next/`
- [X] T170 امشِ سيناريوهات [quickstart.md](./quickstart.md) الستة عشر يدوياً وسجّل أي فارق

> **ستة أثناء التنفيذ:**
>
> - **جدول نتائج واحد لا جدولان، وصفٌّ يُكتب حتى حين لا يجد شيئاً.** `credit_reconciliation_runs`
>   يحمل العدّادات وعيّنة النتائج (`json`) معاً. والتشغيل النظيف يُسجَّل: مشتقّاً من النتائج
>   وحدها، تتساوى منصةٌ سليمة وكنسةٌ توقّفت الثلاثاء الماضي — والثانية هي التي تستحقّ أن
>   يُوقَظ لها أحد، ولذلك تحمل الاستجابة **وقت آخر تشغيل**. والعيّنة مسقوفة عند ٢٠٠ بينما
>   `findings_count` هو العدد الحقيقي، والوظيفة **تسجّل** أنها اقتطعت.
> - **`AdjustCredits` بلا أي سطح HTTP، وهذا ما كشفه تدقيق `T167`.** الصلاحية
>   `billing.credits.adjust` موجودة والفعل يفرض السبب الإلزامي ومفتاح التعامُد، لكن **لا مسار
>   ولا صفحة لوحة** تناديه — فالتصحيح اليوم عملية `tinker`. مكتوبة في `docs/README.md` بدل
>   تركها لمن يكتشفها: الفجوة في **السطح** لا في القاعدة، ولا مهمّة في القائمة تطلب بناءه.
> - **`/orders/*` كانت بلا محدِّد معدّل إطلاقاً**، وهي مسار المال الذي ينتهي عليه شراء
>   الأرصدة: رفع الإيصال يكتب ملفاً، والاعتماد يمنح تسجيلاً ويحرّك رصيداً. أُضيف
>   `throttle:billing` — مسمّى لا سطري، للسبب نفسه في كل مجموعة أخرى. وبقيّة التدقيق نظيفة:
>   كل `throttle:` في المستودع مسمّى، والمطابقات الثلاث الأخرى تعليقات تشرح المنع.
> - **الانتهاء يطالب دفعته أولاً ثم يُرحّل بـ`drawsFromLots: false`** — معامل جديد على
>   `CreditMovement`. بلا ذلك يأخذ السحّاب الأرصدة نفسها مرّةً ثانية من دفعاتٍ **لم تنتهِ**،
>   فيخسر الطالب ضعف ما انتهى. والوظيفة **لا تجد شيئاً اليوم**: `validity_days` افتراضها
>   `null` (`Q-5`)، وترتيب «الأقرب انتهاءً أولاً» حيٌّ منذ اليوم الأول كي لا يعيد تفعيلها
>   لاحقاً كتابةَ أيّ دفعةٍ دفعت ثمن أيّ حصة ماضية.
> - **الخمول احتاج عموداً (`notified_dormant_at`) ونوع إشعار جديداً.** الإشعار **لا يحرّك
>   رصيداً**، فمُسنَدٌ على `last_transaction_at` وحده يعيد إرساله كل ليلة إلى الطالب الأقلّ
>   رغبةً فيه. يُختَم **قبل** الإرسال (تذكيرٌ ضائع أهون من تذكيرٍ كل ليلة أبداً)، ويمحوه
>   الدفتر عند أول حركة. و`CreditBalanceDormant` نوعٌ مستقلّ بقالبه — و**حارسا الإشعارات
>   سقطا كما ينبغي**: `ChannelContractTest` لأن القالب رفض الرسم بلا `months`،
>   و`GuardianDeliveryTest` لأن عدد الأنواع التي تبلغ وليّ الأمر تغيّر — وهو تأكيدٌ مثبَّت
>   عمداً كي يكون رفعُه فعلاً بسبب مكتوب.
> - **فحص المقاعد أُعيدت كتابته قراءتين مجمَّعتين لا استعلامين لكل حصة.** الصياغة البديهية —
>   امشِ الحصص المشحونة واعدد لكلٍّ مقاعدها وقيودها — هي `2N` استعلاماً على الجدول الذي
>   وُجدت الوظيفة كي لا تُثقله، وتنمو مع المنصة. مجمَّعاً هو الجواب نفسه في استعلامين.
> - **البذرة تحوّل مساحة العمل إلى نمطٍ مؤجِّل، وحدها.** في النمط الافتراضي لا شيء ينزل تحت
>   الصفر (`FR-014`)، فالطالب المحجوب يصير غير مميَّز عمّن لم يشترِ شيئاً، ويصير الحدّ
>   الائتماني والموافقة ونافذة الامتحانات أرقاماً لا تفعل شيئاً. وأُضيف
>   `tests/Feature/Seeding/ScenarioSeederTest.php`: البذرة هي الملف الوحيد الذي لا يمرّ به
>   طلبٌ ولا وظيفة ولا اختبار، فيتعطّل صامتاً حتى يعيد أحدهم بناء قاعدته — وهي أسوأ لحظة
>   لاكتشافه.

> **`T169` — وعطلٌ في المنتج كشفه، لا في الاختبار:** تذييل الشريط الجانبي (الحساب وتسجيل
> الخروج) كان `absolute bottom-0` فوق قائمةٍ تمرّر، فكان **يعترض نقرات آخر روابط القائمة**
> على شاشة هاتف: روابط الإدارة مرئية وقابلة للتركيز و**لا يمكن نقرها** — وهو رابط غير موجود
> يرتدي زيّ رابط. صار `<aside>` عمود `flex`، والتذييل ابناً فيه لا لوحةً فوقه، والقائمة وحدها
> هي ما يمرّ. **ولم يكن لأي شاشة من شاشات هذه المرحلة اختبار e2e أصلاً** — لا مهمّة تطلبه —
> فأُضيف `frontend/e2e/billing.spec.ts`: ثلاثة مسارات من **الشريط الجانبي** لا بـ`goto`،
> ويؤكّد كلٌّ منها **ألّا رقم مال على الشاشة**. النتيجة: **569 ناجحاً · 190 متخطّى · صفر
> فاشل** على بناء إنتاجي. و`discovery.spec.ts` المعدَّل منذ مراحل شُغِّل هنا أخيراً.

> **`T170` — السيناريوهات الستة عشر، ومَن يثبت كلاً منها:**
>
> | # | يثبتها |
> |---|---|
> | ١ حساب واحد وأرصدة بالكورس | `AccountContextTest` · `PlatformOwnershipTest` |
> | ٢ الرصيد = مجموع قيوده | `LedgerInvariantTest` · `ReconciliationTest` |
> | ٣ القيد لا يُعدَّل | `ImmutableLedgerTest` |
> | ٤ التزامن | `ConcurrentConsumeTest` · `IdempotencyTest` |
> | ٥ السلسلة كاملة | `ConsumptionChainTest` · `NoChargeTest` |
> | ٦ المقعد لا الحضور | `SeatNotAttendanceTest` · `AttendanceHasNoFinancialEffectTest` |
> | ٧ العتبات لا تتكرّر | `ThresholdNotificationTest` |
> | ٨ الحجب يُرفع بلا تدخّل | `WithholdingTest` · `HighValueAssetTest` |
> | ٩ الأنماط | `BillingModeTest` · `SingleSourceOfModeTest` |
> | ١٠ التسعير واللقطة | `CreditPurchaseFlowTest` (اللقطة تصمد بعد اعتماد سعر جديد) |
> | ١١ سعر المدرّس خرج من العام | `PublicExposureTest` + `/pricing` أُعيدت صياغتها على الحزم |
> | ١٢ الحدود | `ExamModeTest` · `TermsConsentTest` · `CreditLimitTest` |
> | ١٣ حصة سُلِّمت ولم تُشحَن | `UnbilledDeliveryTest` |
> | ١٤ الأرصدة والدفعات | `LotAllocationTest` · فحص `lot_remainder` في المطابقة |
> | ١٥ السكّ بيد المنصة | `CreditMintingTest` · `CreditPurchaseApprovalTest` |
> | ١٦ تعبئة `course_id` | `CourseDeliveryStampTest` + الفرض في `ScheduleClassSession` و`StoreClassSessionRequest` |
>
> **الفارق الوحيد**: §٢ تطلب «١٠٬٠٠٠ معاملة متسلسلة». `LedgerInvariantTest` يمشي عشرات لا
> آلافاً — العدد يقيس زمن التشغيل لا القاعدة، والقاعدة نفسها (`الرصيد = مجموع القيود`) تُفحَص
> الآن **على كل الصفوف** ليلياً بـ`ReconcileCreditBalancesJob`، وهو المكان الذي يُقاس فيه
> الحجم فعلاً. و§١٢ «مدرّس يقرأ رصيد طالب غير مسجَّل عنده» مغطّاة في `CreditLimitTest`
> بالاتجاهين.
