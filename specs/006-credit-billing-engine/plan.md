...
| المبدأ | الحالة | كيف |
|---|---|---|
| **I — طبقات الملكية الثلاث** | ✅ (بعد تعديل دستوري) | كل جدول مُصنَّف في `data-model.md §1`، وجداول الجسر تحمل السمة بتجاوزٍ صريح واحد (سابقة `Enrollment`). **والتعارض حول «حزم الأرصدة» حُسم بتعديل الدستور إلى v1.2.0** بإجراء الحوكمة — راجع `Complexity Tracking` |
...
**نتيجة إعادة الفحص بعد التصميم**: ⚠️ **مخالفة واحدة، سُجّلت ثم حُسمت** بتعديل دستوري في
2026-08-08 — لا `Complexity Tracking` فارغاً كما كتبتُ أولاً، ولا تمريراً بادّعاء «لا مخالفة».
...
| المخالفة | لماذا | كيف حُسمت |
|---|---|---|
| ~~**`credit_packages` مملوكة للمنصة** بينما الدستور v1.1.0 سطر ٥٢ يعدّ «حزم الأرصدة» مملوكةً لمساحة العمل~~ | `FR-016` و`Q-1`: التسعير `cost-plus` تملكه المنصة، والمدرّس **يُمنع** من تعريف سعر أو رؤيته. حزمةٌ يملكها المدرّس تنقض ذلك في جذره | ✅ **تعديل الدستور إلى v1.2.0** (2026-08-08) بإجراء الحوكمة. والتعديل ليس نقل بندٍ بين قائمتين: الطبقة المنصّية صارت **صنفين** — هوية الطالب · وبيانات مرجعية تعرّفها المنصة، والثاني كان قائماً في الكود (`PlatformSetting`) بلا نصّ يغطّيه. والبديل — إبقاؤها لمساحة العمل — مرفوض لأنه إمّا يعيد سعر البيع إلى يد المدرّس (نقضاً لـ`Q-1` كلّها)، وإمّا يُبقي السعر منصّياً ويكرّر القالب بعدد المدرّسين فيُستوفى الدستور على الورق ويُنقض في الجوهر |

**والقرارات المفتوحة**: `courses.price` **حُسم** 2026-08-08 (يبقى مُجمَّداً محصوراً في
`orders.kind = course`؛ التفصيل في [tasks.md T002](./tasks.md))، و`class_sessions.course_id`
**حُسم** (العمود يبقى قابلاً للإفراغ والوجود يُفرَض في `ScheduleClassSession`). ويبقى واحدٌ
**بنيوياً غير قابل للإزالة** ومحلّه [research.md](./research.md): فرق السعر بين الشراء
والتنفيذ — يُعرَض قبل الاعتماد بدل أن يُكتشَف عند الإقفال (`T097`).
