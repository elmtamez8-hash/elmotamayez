...
معلوم السعر، فالرصيد يُشترى لكورس ويُستهلك في حصصه. وبهذا يُحلّ السعر **بنفس الدالة وبنفس
المُدخَلات** عند الشراء وعند التسوية، فيتساوى المحصَّل والمدفوع بالبناء. ويقتضي ذلك ثلاثة
تغييرات على كود منشور: `class_sessions.course_id` يصير إلزامياً، و`courses` تحمل `subject_id`
و`grade_level`، والحصة ترث مادتها من كورسها.
...
**Scale/Scope**: **توسعة `Payments`** بـ٨ جداول جديدة، ~١٢ فعلاً، ٣ عقود مشتركة (اثنان
جديدان وواحد يُوسَّع)، ١٣ مساراً، وثلاث شاشات — إضافةً إلى **خمسة تغييرات على كود منشور**:
`courses` (عمودان + `is_high_value` على `lessons`)، و`class_sessions.course_id` إلزامياً
بهجرة تعبئة تسبق القيد، وإخراج `hourly_rate` من أسطح 001 العامة.
